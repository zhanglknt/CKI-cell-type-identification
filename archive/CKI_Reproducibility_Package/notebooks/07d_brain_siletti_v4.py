...
4. Normalize each pseudobulk: log1p(pb / sum * 1e4) -> raw vector; js_divergence applies softmax internally (single application)
...
# 1. Paths
# ============================================================
# Portable package paths (mirror edition): resolve data/ and results/
# via _paths (package-local with project fallback) instead of the
# hard-coded absolute paths of the main-repository edition.
from _paths import DATA_DIR, RESULTS_DIR, HK_FILE, BRAIN_FILE
H5AD_FILE = BRAIN_FILE
...
# CKI contract (same as 08d): pseudobulks are kept as raw log1p vectors;
# js_divergence() applies softmax internally exactly once.
pseudobulk_log = {}  # (ct, region) -> raw log1p(normalized) vector
...
for key, pb in pseudobulk_raw.items():
    ...
    # Step 2: log1p (softmax happens INSIDE js_divergence — single application)
    pb_log = np.log1p(pb_norm)
    
    pseudobulk_log[key] = pb_log

print(f"  Prepared: {len(pseudobulk_log)} pseudobulks (raw log1p; single softmax inside js_divergence)")
...
ct_to_regions = {}
for (ct, region) in pseudobulk_log.keys():
...
for ct, regions in ct_to_regions.items():
    ...
    for i in range(n_r):
        for j in range(i + 1, n_r):
            ...
            pb_i = pseudobulk_log[(ct, r_i)]
            pb_j = pseudobulk_log[(ct, r_j)]
