...
# Updated 2026-08-30 for corrected extraction (sort_order fix): 55/2120/6149 -> 39/1171/5381
tier_counts_obs = res["tier"].value_counts().to_dict()
assert tier_counts_obs.get("Strong", 0) == 39
assert tier_counts_obs.get("Moderate", 0) == 1171
assert tier_counts_obs.get("Weak", 0) == 5381
...
for name, R_obs, vb in [
    ...
    ("Strong ∧ raw p<0.05（稿件报告的 31 个候选）", STRONG_P05_OBS, combined_null),
]:
    Vbar = float(vb.mean())
    Vmax = int(vb.max())
...
md.append("")
md.append("解读：block-shuffle null 破坏区域结构后，绝对阈值规则（ω<15/25/35 + 残差阈值）"
          "在 null 下反而产生更多通过者——Strong 规则的 null 期望计数达 148.3"
          "（经验 FDR 3.80，P(null≥39) = 1.0），Strong ∧ p<0.05 的 null 期望 131.2 vs "
          "观测 31（经验 FDR 4.23）。即 tier 阈值规则相对 null 并无富集；这与稿件"
          "“候选目录为假设生成列表”的定位一致，且是比“BH q<0.05 不可达"
          "（全族 min q = 0.520）”更直接的经验论证。")
md.append("")
...
md.append("方法：把 31,764 对按细胞类拆成 10 个族，各自独立做 BH（族定义改为最有利的"
          "事后分层）。结果：仅 Bergmann glia 族（m = 21）有 4 对 q < 0.05"
          "（min q = 0.042，全部为小脑内区域间比较），其余 9 个族 min q ≥ 0.28、"
          "无一对 q < 0.05。即除小脑内比较外，“q<0.05 不可达”的结论对族定义稳健；"
          "小脑内 4 对与稿件分层 BH 的 Bergmann 族发现一致。")
md.append("")
...
md.append("方法：以**区域**（108 个，成对统计量的设计单元；10x library/sample 嵌套于区域内，"
          ...
          "取 2.5/97.5 百分位。单位权重下精确复现观测值（gradient 6.10、Strong 39、"
          "Astrocyte 82.75），已作断言校验。")
md.append("")
...
md.append("方法：对 `brain_bs_null_ct_test.csv` 的 10 个类级置换检验 p 值执行 BH。"
          "未校正下 4/10 类显著：Astrocyte p = 0.0010（SES 6.49）、Oligodendrocyte "
          "precursor p = 0.0020（SES 4.04）、Committed OPC p = 0.0050（SES 2.60）、"
          "Fibroblast p = 0.0300（SES 1.83）；Ependymal 边缘（p = 0.075，SES 1.52）。"
          "BH 后 3/10 类 q < 0.05（Astrocyte q = 0.010、OPC q = 0.010、cOPC q = 0.017；"
          "Fibroblast q = 0.075）。三个类的观测均值反而低于 null：Oligodendrocyte "
          "（p = 0.883，SES −1.15）、Microglia（p = 0.904，SES −1.25）、Bergmann glia "
          "（p = 0.998，SES −2.76）；Choroid plexus p = 0.566、Vascular p = 0.115。"
          "双侧与 leave-pair-out 固定基因面板敏感性下 Astrocyte/OPC/cOPC 的 q ≤ 0.033 "
          "保持稳健，Fibroblast 在固定面板下 q = 0.050。")
md.append("")
md_table(df_bh, floatfmt=4)
md.append("")
classlevel_conclusion = (
    "类级显著性结论（供稿件表述）：block-shuffle null 下 4/10 类显著（Astrocyte、"
    "Oligodendrocyte precursor、Committed OPC、Fibroblast），BH 后 3/10（q = 0.010/"
    "0.010/0.017），且在双侧与固定基因面板敏感性下稳健；Fibroblast q = 0.075 边缘。"
    "Ependymal 边缘未校正（p = 0.075）。注意三个类（Oligodendrocyte、Microglia、"
    "Bergmann glia）观测均值低于 null（SES 为负），不得表述为显著；早期版本中 "
    "Bergmann glia 的类级显著性（p = 0.031、q = 0.034）源于错误的行分配提取，"
    "修复后 p = 0.998，该结论作废。稿件应表述为“4/10 类显著、BH 后 3/10，"
    "但无单个候选对在校正后存活”。"
)
md.append(classlevel_conclusion)
md.append("")
...
md.append("")
md.append("1. 1a 的全局族在 α = 0.05 下 R = 1,960 略多于 null 期望 V̄ ≈ 1,588"
          "（经验 FDR 0.81），富集幅度有限；全族最小 q = 0.520，无单个候选对在校正后"
          "存活——这是稿件“候选目录为假设生成列表”定位的重抽样版本。")
md.append("2. 1b 的选择规则 FDR 是对候选目录最直接的经验 FDR 陈述：Strong 规则的 null "
          "期望计数为 148.3（经验 FDR 3.80，P(null≥39) = 1.0），稿件报告的 31 个 "
          "Strong ∧ p<0.05 候选的 null 期望为 131.2（经验 FDR 4.23）——null 下通过数"
          "反而远多于观测，tier 阈值规则相对 null 无富集，候选目录应保持"
          "“假设生成”定位；1c 表明该结论对族定义（按类分层）稳健"
          "（仅 Bergmann glia 小脑内 4 对 q < 0.05）。")
md.append("3. 2 的聚类 bootstrap CI 显著宽于稿件原有的对级重抽样 CI（R2-M2 所指"
          "“CI 系统性过窄”），建议正文/图注全部替换为本表口径。")
md.append("4. 3 的类级 BH 表与稿件类级显著性陈述一致：4/10 类未校正显著、BH 后 3/10；"
          "早期版本的 Bergmann glia 显著性（p = 0.031）源于错误提取，修复后 p = 0.998，"
          "该结论作废。")
md.append("")
...
log(f"gradient_fold 6.10  CI:             [{df_bb.iloc[0]['CI_low']}, {df_bb.iloc[0]['CI_high']}]")
log(f"Astrocyte mu_ct 82.75 CI:           see block_bootstrap table")
