...
md.append("**影响判断：支持（结论稳健，但需加限定语）**。排除 Bergmann 后梯度仍 "
          f"{grad_excl:.2f} 倍、CI 下限 {ci_lo:.2f}，远大于 1；全 10 类梯度 {grad_all:.2f}x 与排除后 "
          f"{grad_excl:.2f}x 一致（Bergmann 与 Vascular 均值并列最低），梯度低端稳健，"
          f"摘要宜按 R3 建议改为 "
...
md.append("**方法**：对同一批 31,764 对计算各类 k_f 均值与 k_n 均值，"
          ...
          f"并对 {grad_all:.2f} 倍端点（astrocyte vs Bergmann glia）做乘法分解 {grad_all:.2f} ≈ (Δk_f)×(Δk_n)。\n")
md.append("\n| 量 | 数值 |\n|---|---|")
...
md.append(f"**影响判断：需修正（证实共识 1）**。{grad_all:.2f} 倍梯度的端点对比中 k_f 仅贡献 {ep_kf:.2f} 倍、k_n 贡献 "
          f"{ep_kn:.2f} 倍（乘积 {ep_pred:.2f}≈{grad_all:.2f}）；k_f-only 排序下 astrocyte 仅列第 {astro_rank_kf}，"
...
md.append(f"| 对照：脑区数据（kn_floor=0）k_n<1×10⁻⁴ 对数 | {brain_below} / 31,764（与稿件声明一致） |")
...
md.append(f"| A1 | 排除 Bergmann 梯度 | {grad_all:.2f}x → **{grad_excl:.2f}x**（95% CI [{ci_lo:.2f}, {ci_hi:.2f}]） | 支持（加限定语） |")
