...
cats = ['C (control split)', 'S (same ct, diff organ)', 'D (diff ct, same organ)', 'X (diff ct, cross organ)']
...
# Panel D: ω by comparison category (REAL DATA from mouse_pilot_v2_results.csv)
# C = control split; S = same cell type, different organ; D = different cell
# type within same organ; X = different cell type, cross-organ.
axD = fig.add_subplot(gs[1, :])

_pilot_v2 = pd.read_csv(RESULTS_DIR / 'mouse_pilot_v2_results.csv')
_cat_order_d = ['C_control', 'S_same_ct', 'D_diff_ct', 'X_cross']
_cat_labels_d = ['C (control split)', 'S (same cell type,\ndifferent organ)',
                 'D (different cell type,\nsame organ)', 'X (different cell type,\ncross-organ)']
_omega_groups = [_pilot_v2.loc[_pilot_v2['category'] == c, 'omega'].values for c in _cat_order_d]
_ns_d = [len(g) for g in _omega_groups]
bp = axD.boxplot(_omega_groups, patch_artist=True, widths=0.5,
                 showfliers=True, flierprops=dict(marker='.', markersize=3, alpha=0.5))
for patch, col in zip(bp['boxes'], [C_BLUE, C_GREEN, C_AMBER, C_RED]):
    patch.set_facecolor(col); patch.set_alpha(0.7)
for med in bp['medians']:
    med.set_color('black'); med.set_linewidth(1.2)
axD.set_yscale('log')
axD.set_xticks(range(1, len(_cat_labels_d) + 1))
axD.set_xticklabels([f'{lab}\n(n = {n})' for lab, n in zip(_cat_labels_d, _ns_d)], fontsize=8)
axD.set_ylabel('ω (log scale)', fontsize=8)
axD.set_title('ω increases monotonically with biological distance (mouse pilot)',
              fontsize=8, fontweight='bold')
# One-sided Mann-Whitney: X vs C (calibration contrast) — real data
from scipy.stats import mannwhitneyu as _mwu
_u, _p_xc = _mwu(_omega_groups[3], _omega_groups[0], alternative='greater')
axD.text(0.02, 0.95, f'X vs. C: one-sided Mann-Whitney P = {_p_xc:.1e}',
         transform=axD.transAxes, fontsize=8, va='top',
         bbox=dict(facecolor='white', edgecolor='gray', alpha=0.8, pad=2))
print(f"  Panel D: omega by category (n={_ns_d}), X vs C P={_p_xc:.2e}")
...
# Panel D: ω by pair category (REAL DATA)
# Category convention (consistent with mouse pilot, Fig. 2):
#   C = same cell type, same organ (control-like; n = 0 in this dataset)
#   S = same cell type, different organ
#   D = different cell type, same organ
#   X = different cell type, different organ
axD = fig.add_subplot(gs[1, 0])
_hpairs_all = pd.read_csv(RESULTS_DIR / 'phase33_v3_human_pairs.csv')
_hpairs_all['cat_4way'] = 'X'
...
_hpairs_all.loc[_hpairs_all['same_organ'] & ~_hpairs_all['same_ct'], 'cat_4way'] = 'D'
cats_4way = ['C', 'S', 'D', 'X']
cat_labels_full = ['C\n(same ct,\nsame organ)', 'S\n(same ct,\ndiff organ)',
                   'D\n(diff ct,\nsame organ)', 'X\n(diff ct,\ndiff organ)']
_box_data = []
...
# Drop empty categories (C has no pairs in this dataset) — keep label alignment
_keep = [i for i, d in enumerate(_box_data) if len(d) > 0]
_box_data = [_box_data[i] for i in _keep]
cat_labels_full = [cat_labels_full[i] for i in _keep]
_cats_kept = [cats_4way[i] for i in _keep]
bp = axD.boxplot(_box_data, labels=cat_labels_full, patch_artist=True, showfliers=False)
for patch, c in zip(bp['boxes'], [CAT_COLORS[k] for k in _cats_kept]):
...
# Panel B: ω gradient (10 cell classes) — REAL DATA (block-shuffle null results, authoritative)
axB = fig.add_subplot(gs[0, 1:])
_bs_res = pd.read_csv(RESULTS_DIR / 'brain_bs_null_results.csv')
_brain_ct_bs = (_bs_res.groupby('cell_type')['omega'].mean()
                .sort_values().reset_index())
_brain_ct_bs.columns = ['cell_type', 'omega_mean']
cell_classes = _brain_ct_bs['cell_type'].values
omega_vals = _brain_ct_bs['omega_mean'].values
...
# Panel C: Brain region heatmap (astrocyte — REAL DATA, block-shuffle results)
axC = fig.add_subplot(gs[1, :])
# Build region × region omega matrix from brain_bs_null_results.csv
_brain_astro = _bs_res[_bs_res['cell_type'] == 'Astrocyte']
...
# Panel D: Migration candidates (REAL DATA — block-shuffle tier counts)
axD = fig.add_subplot(gs[2, 0])
_tier_ct = _bs_res['tier'].value_counts()
_n_strong = int(_tier_ct.get('Strong', 0))
_n_moderate = int(_tier_ct.get('Moderate', 0))
_n_weak = int(_tier_ct.get('Weak', 0))
_n_total = len(_bs_res)
mig_levels = [f'Strong ({_n_strong})', f'Moderate ({_n_moderate:,})', f'Weak ({_n_weak:,})']
mig_pct = [100.0 * _n_strong / _n_total,
           100.0 * _n_moderate / _n_total,
           100.0 * _n_weak / _n_total]
...
# (authoritative block-shuffle null results)
_mig = pd.read_csv(RESULTS_DIR / 'brain_bs_null_results.csv')
...
if _hk_stab_file.exists():
    ...
    raise FileNotFoundError("hk_stability_sweep.csv not found — run notebooks/01b_hk_stability.py "
                           "(no fallback: fabricated data not acceptable)")
axA = fig.add_subplot(gs[0, 0])
...
# Panel B: per-module GSVA score variance across cell types (REAL DATA)
# Data source: phase32_pathway_scores.csv (produced by 04_phase32_sweep.py)
# 20 HVG-partition modules; variance of module GSVA scores across 38 CT entries.
# Shows that cell-type divergence is distributed across modules rather than
# concentrated in a single module (supports identity-only k_f configuration).
_pw_score_file = RESULTS_DIR / 'phase32_pathway_scores.csv'
axB = fig.add_subplot(gs[0, 1])
if _pw_score_file.exists():
    _pw_scores = pd.read_csv(_pw_score_file, index_col=0)
    _mod_var = _pw_scores.var(axis=0).sort_values(ascending=False)
    _mod_labels = [f'M{m}' for m in _mod_var.index]
    _mod_vals = _mod_var.values
    print(f"  Panel B: loaded per-module variance from {_pw_score_file} "
          f"({_pw_scores.shape[0]} CTs x {_pw_scores.shape[1]} modules)")
else:
    raise FileNotFoundError("phase32_pathway_scores.csv not found — run 04_phase32_sweep.py "
                            "(no fallback: fabricated data not acceptable)")
axB.bar(range(len(_mod_vals)), _mod_vals, color=C_GREEN, alpha=0.8)
axB.set_xlabel('HVG module (sorted by variance)', fontsize=7, labelpad=3)
axB.set_ylabel('GSVA score variance\nacross cell types', fontsize=7, labelpad=3)
axB.set_title('Divergence distributed across HVG modules', fontsize=7, pad=6)
axB.set_xticks(range(0, len(_mod_vals), 4))
axB.set_xticklabels([_mod_labels[i] for i in range(0, len(_mod_vals), 4)], fontsize=7)
add_panel_label(axB, 'B', col_pos='left')
...
if _sweep_file.exists():
    ...
    _top4 = _sweep.head(4)
    sweep_params = ['CKI ω\n(identity)'] + [f'+pathway\n{w2:.1f}' for w2 in _top4['w2'].values[1:]]
    sweep_auc = list(_top4['auc'])
    print(f"  Panel C: loaded sweep AUC from CSV: {[f'{a:.3f}' for a in sweep_auc]}")
else:
    raise FileNotFoundError("phase32_sweep_results.csv not found — run 04_phase32_sweep.py "
                           "(no fallback: fabricated data not acceptable)")
...
_ed6_ct = (_mig_all.groupby('cell_type')
           .agg(omega_mean=('omega', 'mean'),
                n_pairs=('omega', 'size'))
           .reset_index())
_nr_map = {ct: len(set(g['region_a']) | set(g['region_b']))
           for ct, g in _mig_all.groupby('cell_type')}
_ed6_ct['n_regions'] = _ed6_ct['cell_type'].map(_nr_map)
_ed6_ct = _ed6_ct.sort_values('omega_mean')
fig = plt.figure(figsize=(DOUBLE, 140*MM), dpi=DPI)
...
_top_ct_nuclei = _brain_ct_nuclei.nlargest(6, 'n_nuclei')
...
# Panel B: ω distribution per cell class (REAL DATA — block-shuffle results)
axB = fig.add_subplot(gs[0, 1:])
classes_br = _ed6_ct['cell_type'].values
_box_br = [_mig_all.loc[_mig_all['cell_type'] == ct, 'omega'].values for ct in classes_br]
bp_br = axB.boxplot(_box_br, patch_artist=True, showfliers=False,
                    labels=[c[:14] for c in classes_br])
for patch in bp_br['boxes']:
    patch.set_facecolor(C_GREEN); patch.set_alpha(0.6)
for med in bp_br['medians']:
    med.set_color('black'); med.set_linewidth(1.2)
axB.set_yscale('log')
axB.set_ylabel('CKI ω (log scale)', fontsize=8)
axB.set_title('ω distribution per cell class (block-shuffle observed landscape)', fontsize=8)
axB.tick_params(axis='x', labelrotation=25, labelsize=8)
...
axC = fig.add_subplot(gs[1, 0])
axC.scatter(_ed6_ct['n_regions'], _ed6_ct['omega_mean'],
            c=C_PURPLE, s=40, alpha=0.8, edgecolors='none')
r_sp, p_sp = spearmanr(_ed6_ct['n_regions'], _ed6_ct['omega_mean'])
...
# Panel A: Residual distribution (REAL DATA — block-shuffle null results, authoritative)
axA = fig.add_subplot(gs[0, 0])
_mig_all = pd.read_csv(RESULTS_DIR / 'brain_bs_null_results.csv')
