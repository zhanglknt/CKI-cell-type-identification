#!/usr/bin/env python3
"""Build CKI Submission Package v39 (Genome Biology).
...
V38_ZIP = VERSION3_DIR / "CKI_Submission_v39.zip"
WORK_DIR = VERSION3_DIR / "CKI_Submission_v39"
...
def build_v38():
    print("=" * 60)
    print("  CKI Submission Package v39 (Genome Biology) Builder")
    ...
    # 3. Manifest
    print(f"\n[3] Writing MANIFEST_v39.txt...")
    manifest = f"""CKI Submission Package v39 (Genome Biology, Methodology article)
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Status: Second simulated peer-review revision (v37 report, 5 reviewers,
4.4/10 Major Revision) applied; ground-truth simulation, fixed gene-panel
ablation, and engineering-consistency fixes (scripts 44/45/46).
...
"""
    with open(WORK_DIR / "MANIFEST_v39.txt", "w", encoding="utf-8") as f:
        f.write(manifest)
    ...
    with zipfile.ZipFile(V38_ZIP, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(WORK_DIR):
            for fn in sorted(files):
                fp = Path(root) / fn
                zf.write(fp, f"CKI_Submission_v39/{fn}")

    zip_mb = V38_ZIP.stat().st_size / (1024 * 1024)
    print(f"\n{'='*60}")
    print(f"  v39 (GB) Package: {V38_ZIP}")
    ...
    if v.failed == 0:
        print(f"\n  *** ALL {v.passed} CHECKS PASSED — v39 (GB) FINAL ***")
    else:
