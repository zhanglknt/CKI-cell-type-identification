#!/usr/bin/env python3
"""Build CKI Submission Package v44 (Genome Biology).
...
V38_ZIP = VERSION3_DIR / "CKI_Submission_v44.zip"
WORK_DIR = VERSION3_DIR / "CKI_Submission_v44"
...
def verify_r2_additions(v: Verifier):
    ...
    s = v.supp_text()
    g = v.rg_text()
    mf = (v.wd / "MANIFEST_v44.txt").read_text(encoding="utf-8")
    import json as _json
...
def verify_v43_reprofix(v: Verifier):
    ...
    v.check("v0.4.7" in cl and "v0.4.6" not in cl,
            "V43-15 cover letter release tag v0.4.7")
    ...
    v.check("cki:0.4.7" in _readme and "cki:0.4.6" not in _readme,
            "V43-17 README docker tag 0.4.7")
    v.check("releases/tag/v0.4.7" in _data_readme,
            "V43-18 data README release link v0.4.7")
...
def verify_v44_additions(v: Verifier):
    ...
    v.check("25,876" in t and "13.555" in t and "13.559" in t,
            "V44-4d MS threshold sensitivity + Bergmann/Vascular near-tie")
...
def verify_v38_reviewer_fixes(v: Verifier):
    ...
    v.check(bool(re.search(r'v0\.4\.7|0\.4\.7', t)) and bool(re.search(r'v0\.4\.7|0\.4\.7', rg))
            and not re.search(r'0\.4\.[23456]', t) and not re.search(r'0\.4\.[23456]', rg),
            "V38-3g version 0.4.7 in MS + guide (no 0.4.2-0.4.6 residue)")
    _pkg_init = (BASE_DIR / "cki" / "__init__.py").read_text(encoding="utf-8")
    _pyproject = (BASE_DIR / "pyproject.toml").read_text(encoding="utf-8")
    v.check('__version__ = "0.4.7"' in _pkg_init
            and re.search(r'^version\s*=\s*"0\.4\.7"', _pyproject, re.M) is not None,
            "REL-1 package version bumped to 0.4.7 in cki/__init__.py + pyproject.toml")
...
def build_v38():
    print("=" * 60)
    print("  CKI Submission Package v44 (Genome Biology) Builder")
    print("  Blind-review round-3 fixes + fresh DOCX rebuild")
    print("=" * 60)
    ...
    # 3. Manifest
    print(f"\n[3] Writing MANIFEST_v44.txt...")
    manifest = f"""CKI Submission Package v44 (Genome Biology, Methodology article)
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Status: v44 = v43 + blind-review round-3 fixes (2026-09-04; four-expert
panel, mean 7.25/10; all P1 items resolved): TCGA probability-mapping
disclosed and re-run under linear normalization (all qualitative
conclusions preserved); brain k_n class-size confound controlled
(equal-n downsample: gradient direction retained at 1.74 [1.64, 1.84]);
min-cells threshold sensitivity disclosed; mouse omega calibration
updated to 7.70 [7.37, 8.02] (50 split-half replicates, 300 values;
legacy 6.67 superseded, CIs overlap); competitor benchmarking added
(MELD 1.0.2 + Python approximation of scDist; anchor-visibility
boundary quantified, working range ~50-200 cells per donor per
condition); cki package bumped to v0.4.7 (tail parameter, ses naming,
kn_floor exposed, densify warning, unified guards, pseudobulk
group-size fix); seed 20260903 exceptions disclosed; GSE96583 +
self-contained CELLxGENE collection ID + Additional file 2 declared.
Package released as tag v0.4.7.
...
"""
    ...
    for _name in _fig_names:
        ...
        if _fp.exists():
            _h = _hashlib.sha256(_fp.read_bytes()).hexdigest()
            manifest += f"  {_h}  {_name} ({_fp.stat().st_size:,} bytes)\n"
    with open(WORK_DIR / "MANIFEST_v44.txt", "w", encoding="utf-8") as f:
        f.write(manifest)
        ...
        for root, dirs, files in os.walk(WORK_DIR):
            for fn in sorted(files):
                if ZIP_EXCLUDE(fn):
                    continue
                fp = Path(root) / fn
                zf.write(fp, f"CKI_Submission_v44/{fn}")

    zip_mb = V38_ZIP.stat().st_size / (1024 * 1024)
    print(f"\n{'='*60}")
    print(f"  v44 (GB) Package: {V38_ZIP}")
    ...
    # 5. Verification
    print(f"\n{'='*60}")
    print(f"  v44 Final Verification")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"  v44 Verification Summary")
    print(f"{'='*60}")
    ...
    if v.failed == 0:
        print(f"\n  *** ALL {v.passed} CHECKS PASSED — v44 (GB) FINAL ***")
    else:
