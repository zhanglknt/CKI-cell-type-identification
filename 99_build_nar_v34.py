#!/usr/bin/env python3
"""Build CKI NAR Submission Package v34.

v34: v33 fourth-batch expert panel (8.64/10) — 3 Major + 3 Minor ALL resolved.
  Major: M1(Repro Guide §6→§2.1 parameter table) M3(S1-S12 in-text refs) M4(7 orphan refs cited)
  Minor: m8(permutation support wording) m9(Limitation #20 sequential) m17(orthogonal→complementary)

Previous fixes inherited:
  v33: v32 third-batch expert panel (8.41/10) — 4 Major + 20 Minor, Abstract 195 words
...
Files regenerated:
  - CKI_NAR_Manuscript.docx (fresh, v34 M3/M4/m8/m9/m17 fixes)
  - CKI_NAR_Supplementary.docx (fresh)
  - CKI_NAR_Cover_Letter.docx (fresh)
  - CKI_NAR_Reproducibility_Guide.docx (fresh, M1 §6→§2.1 fix)
  - Table1-2.docx (fresh)
"""
...
V34_ZIP = VERSION3_DIR / "CKI_NAR_Submission_v34.zip"
WORK_DIR = VERSION3_DIR / "CKI_NAR_Submission_v34"
V33_DIR = VERSION3_DIR / "CKI_NAR_Submission_v33"
...
def verify_p2_e3(v: Verifier):
    ...
    ordinal_count = len(re.findall(r'(?:First|Second|Third|Fourth|Fifth|Sixth|Seventh|Eighth|Ninth|Tenth|Eleventh|Twelfth|Thirteenth|Fourteenth|Fifteenth|Sixteenth|Seventeenth|Eighteenth|Nineteenth|Twentieth|Twenty.first)', lim_section))
...
# -----------------------------------------------------------
# v34 checks — Fourth-batch Expert Panel (3 Major + 3 Minor)
# -----------------------------------------------------------

def verify_v34_expert_panel(v: Verifier):
    print(f"\n{'─'*50}")
    print(f"  v34: Fourth-Batch Expert Panel (8.64/10) — 3 Major + 3 Minor")
    print(f"{'─'*50}")
    ...
    # --- Major (3) ---
    # M1: Repro Guide parameter table moved from §6 to §2.1
    ...
    # M3: S1-S12 supplementary figures cited in body
    ...
    # --- Minor (3) ---
    # m8: "both with permutation support" (not "both statistically significant")
    has_perm_support = bool(re.search(r'both\s+with\s+permutation\s+support', t))
    no_stat_sig = "both statistically significant" not in t.lower()
    v.check(has_perm_support and no_stat_sig, "m8 permutation support wording")

    # m9: Limitation #20 exists, numbering sequential (no gap #19→#21)
    lim_section = t.split("Limitations should be noted")[1].split("Future directions")[0] if "Limitations should be noted" in t else ""
    has_20 = "Twentieth" in lim_section
    has_21 = "Twenty-first" in lim_section
    has_nineteenth = "Nineteenth" in lim_section
    v.check(has_nineteenth and has_20 and has_21, f"m9 Limitations sequential (19/20/21 all present)")
...
def build_v34():
    print("=" * 60)
    print("  CKI NAR Submission Package v34 Builder")
    print("  Fourth-batch expert panel (8.64/10) — 3 Major + 3 Minor")
    print("=" * 60)
    ...
    # Unpack v33 ZIP to get figures
    if not V33_DIR.exists() and V33_ZIP.exists():
        print("  Unpacking v33.zip for figures...")
        with zipfile.ZipFile(V33_ZIP, 'r') as zf:
            zf.extractall(VERSION3_DIR)

    src = V33_DIR if V33_DIR.exists() else None
    if src and src.exists():
        ...
        print(f"  ERROR: Source directory v33 not found")
        return False
    ...
    # 2. Manifest
    print(f"\n[2] Writing MANIFEST_v34.txt...")
    manifest = f"""CKI NAR Submission Package v34
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Status: v33 fourth-batch expert panel (8.64/10) — 3 Major + 3 Minor ALL resolved
  + all earlier fixes inherited (v33/v32/v31/v30/v29/v26-v28)

=== v34: Fourth-Batch Expert Panel Fixes (3 Major + 3 Minor) ===
Major (3/3):
  M1: Repro Guide parameter table relocated from Section 6 to Section 2.1
  M3: Supplementary Figures S1-S12 all cited in manuscript body
  M4: 7 orphan references (31-35, 40-41) cited in manuscript body

Minor (3/3):
  m8: Introduction "both statistically significant" -> "both with permutation support"
  m9: Limitations numbering sequential (First through Twenty-first), #20 (k_n floor) present
  m17: "orthogonal" -> "complementary" (2 occurrences, L77 + L116)

=== v33 (inherited): Third-Batch Expert Panel (8.41/10) — 4 Major + 20 Minor ===
  M1(Repro Guide section 6) M2(data source) M3(S3-S9 refs) M4(8 orphan refs)
  m1(FDR terminology) m2(CV 60%->52%) m3(Cover Letter signatures) m4-m17

=== v32 (inherited): Abstract Compression (195 words) ===
...
Contents:
1. CKI_NAR_Manuscript.docx - Main manuscript (v34: M3/M4/m8/m9/m17 fixes)
2. CKI_NAR_Supplementary.docx - Supplementary materials
3. CKI_NAR_Cover_Letter.docx - Cover letter
4. CKI_NAR_Reproducibility_Guide.docx - Reproducibility guide (M1: §2.1 parameter table)
...
"""
    with open(WORK_DIR / "MANIFEST_v34.txt", "w", encoding="utf-8") as f:
        f.write(manifest)

    # 3. ZIP
    print(f"\n[3] Creating ZIP...")
    with zipfile.ZipFile(V34_ZIP, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(WORK_DIR):
            for fn in sorted(files):
                fp = Path(root) / fn
                zf.write(fp, f"CKI_NAR_Submission_v34/{fn}")

    zip_mb = V34_ZIP.stat().st_size / (1024 * 1024)
    print(f"\n{'='*60}")
    print(f"  v34 Package: {V34_ZIP}")
    print(f"  Size: {zip_mb:.1f} MB")
    with zipfile.ZipFile(V34_ZIP, "r") as zf:
    ...
    # 4. Verification
    print(f"\n{'='*60}")
    print(f"  v34 Final Verification")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"  v34 Verification Summary")
    print(f"{'='*60}")
    ...
    if v.failed == 0:
        print(f"\n  *** ALL {v.passed} CHECKS PASSED — v34 FINAL ***")
    else:
