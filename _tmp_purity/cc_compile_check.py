...
files = [
    ...
    r"C:/Users/KnightZ/Desktop/细胞受选择/notebooks/nc49_tcga_purity.py",
...
for f in files:
    try:
        ...
        cc_lusc = '"CC": "TCGA-LUSC"' in src or '"CC":"TCGA-LUSC"' in src
        cc_lihc = '"CC": "TCGA-LIHC"' in src or '"CC":"TCGA-LIHC"' in src
        rg_kirc = '"RG": "TCGA-KIRC"' in src or '"RG":"TCGA-KIRC"' in src
        rg_lihc = '"RG": "TCGA-LIHC"' in src or '"RG":"TCGA-LIHC"' in src
        out.append(f"OK {f.split('/')[-2]}/{f.split('/')[-1]}: CC->LUSC={cc_lusc}, CC->LIHC={cc_lihc}, RG->KIRC={rg_kirc}, RG->LIHC={rg_lihc}")
    except Exception as e:
        out.append(f"FAIL {f}: {e}")
open(r"C:/Users/KnightZ/Desktop/细胞受选择/_tmp_purity/cc_compile_check.txt", "w").write("\n".join(out))
