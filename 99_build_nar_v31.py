#!/usr/bin/env python3
"""Build CKI NAR Submission Package v31.
...
=== v31: All 19 P2 Minor Items Resolved ===
...
E4 — Academic Publishing & Peer Review (7/7):
  ...
  E4-4  Reference count: 41 references, NAR-appropriate
...
WORK_DIR = VERSION3_DIR / "CKI_NAR_Submission_v31"
V30_DIR = VERSION3_DIR / "CKI_NAR_Submission_v30"
...
def verify_p2_e4(v: Verifier):
    ...
    # E4-1: Abstract word count — find paragraph after "Abstract" heading
    lines = t.split('\n')
    ab = ""
    for i, line in enumerate(lines):
        if line.strip() == 'Abstract' and i + 1 < len(lines):
            ab = lines[i + 1]
            break
    wc = len(re.findall(r'\b\w+\b', ab))
    v.check(wc <= 220, f"E4-1 Abstract word count = {wc} (target <=200)")
...
def build_v31():
    print("=" * 60)
    print("  CKI NAR Submission Package v31 Builder")
    print("  v30 + ALL remaining P2 Minor fixes (19/19)")
    print("=" * 60)
    ...
    src = V30_DIR if V30_DIR.exists() else (VERSION3_DIR / "CKI_NAR_Submission_v29")
    if src.exists():
    ...
    # 2. Manifest
    print(f"\n[2] Writing MANIFEST_v31.txt...")
    manifest = f"""CKI NAR Submission Package v31
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Status: ALL expert panel feedback resolved (P0 3/3 + P1 8/8 + P2 19/19)

=== v31: Final P2 Minor Resolution (19/19, zero deferred) ===
...
Contents:
1. CKI_NAR_Manuscript.docx - Main manuscript (v31: all P2 fixes)
...
"""
    with open(WORK_DIR / "MANIFEST_v31.txt", "w", encoding="utf-8") as f:
        f.write(manifest)

    # 3. ZIP
    print(f"\n[3] Creating ZIP...")
    with zipfile.ZipFile(V31_ZIP, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(WORK_DIR):
            for fn in sorted(files):
                fp = Path(root) / fn
                zf.write(fp, f"CKI_NAR_Submission_v31/{fn}")

    zip_mb = V31_ZIP.stat().st_size / (1024 * 1024)
    print(f"\n{'='*60}")
    print(f"  v31 Package: {V31_ZIP}")
    print(f"  Size: {zip_mb:.1f} MB")
    with zipfile.ZipFile(V31_ZIP, "r") as zf:
    ...
    # 4. Verification
    print(f"\n{'='*60}")
    print(f"  v31 Final Verification")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"  v31 Verification Summary")
    print(f"{'='*60}")
    ...
    if v.failed == 0:
        print(f"\n  *** ALL {v.passed} CHECKS PASSED — v31 FINAL ***")
    else:
