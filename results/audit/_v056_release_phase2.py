#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v56 release chain phase-2: Zenodo v0.5.3 record is live
(22954782 / 10.5281/zenodo.22954782; redelivery of the 'released'
webhook unblocked the silently failed async deposit), the manuscript
Code availability now cites the version DOI, and the submission zip
has been rebuilt (phase-2 build, 221/221).

This script replaces the Release v0.5.3 asset (old id 587716572,
...
Adapted from _v054_release_phase2.py (same urllib+PAT method).
"""
import hashlib
import json
import re
...
new_body = rel["body"].replace(
    "the manuscript Code availability section cites the concept DOI, and the v0.5.3 version DOI is written back in the phase-2 commit on main",
    f"the manuscript Code availability section cites the concept DOI and the v0.5.3 version DOI {VERSION_DOI} (phase-2 write-back complete; asset refreshed to the phase-2 build)")
