#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v57 release chain phase-2: Zenodo v0.5.4 record is live
(NEW_RECORD / 10.5281/zenodo.NEW_RECORD), the manuscript Code
availability now cites the version DOI, and the submission zip has
been rebuilt (phase-2 build, 221/221).

This script replaces the Release v0.5.4 asset (old id 588088967,
...
Adapted from _v056_release_phase2.py (same urllib+PAT method).
"""
import hashlib
import json
import re
...
assert "NEW_RECORD" not in VERSION_DOI, "fill VERSION_DOI first"
...
new_body = rel["body"].replace(
    "the manuscript Code availability section cites the concept DOI and the v0.5.3 version DOI 10.5281/zenodo.22954782; the v0.5.4 version DOI is written back in the phase-2 commit on main",
    f"the manuscript Code availability section cites the concept DOI and the v0.5.4 version DOI {VERSION_DOI} (phase-2 write-back complete; asset refreshed to the phase-2 build)")
assert new_body != rel["body"], "body anchor not found - patch aborted"
