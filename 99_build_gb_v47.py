...
def verify_r2_additions(v: Verifier):
    ...
    s = v.supp_text()
    g = v.rg_text()
    mf = (v.wd / "MANIFEST_v47.txt").read_text(encoding="utf-8")
    import json as _json
...
def verify_v43_reprofix(v: Verifier):
    ...
    v.check("v0.5.0" in cl and "v0.4.9" not in cl,
            "V43-15 cover letter release tag v0.5.0 (v47)")
    ...
    v.check("cki:0.5.0" in _readme and "cki:0.4.9" not in _readme,
            "V43-17 README docker tag 0.5.0 (v47)")
    v.check("releases/tag/v0.5.0" in _data_readme,
            "V43-18 data README release link v0.5.0 (v47)")
...
def verify_v46_additions(v: Verifier):
    ...
    # (i) version refs on v0.5.0; MS v0.4.9 only on phase-1 DOI line
...
def verify_v38_reviewer_fixes(v: Verifier):
    ...
    v.check(bool(re.search(r'v0\.5\.0|0\.5\.0', t)) and bool(re.search(r'v0\.5\.0|0\.5\.0', rg))
            and not re.search(r'0\.4\.[2345678]', t) and not re.search(r'0\.4\.[2345678]', rg),
            "V38-3g version 0.5.0 in MS + guide (no 0.4.2-0.4.8 residue; "
            "MS v0.4.9 only on phase-1 Zenodo line)")
    _pkg_init = (BASE_DIR / "cki" / "__init__.py").read_text(encoding="utf-8")
    _pyproject = (BASE_DIR / "pyproject.toml").read_text(encoding="utf-8")
    v.check('__version__ = "0.5.0"' in _pkg_init
            and re.search(r'^version\s*=\s*"0\.5\.0"', _pyproject, re.M) is not None,
            "REL-1 package version = 0.5.0 in cki/__init__.py + "
            "pyproject.toml (v47)")
...
def build_v38():
    ...
    manifest = f"""CKI Submission Package v47 (Genome Biology, Methodology article)
