#!/usr/bin/env python3
"""Build CKI NAR Submission Package v28.

v28 incorporates all v27 fixes AND N10 (Python version consistency).

Changes from v27:
  N10 — Python version consistency resolved (3 places unified to >=3.10)
         Manuscript Data Availability "3.8+" -> ">=3.9" (matches Cover Letter)
         Manuscript Methods declares actual runtime: "Python 3.13.12"
...
All Issues Resolved:
  N1-N10 (P0 + P1 + P2) from v25 expert panel review (Score: 7.50/10)

Inherits from v25-v27:
...
Files regenerated:
  - CKI_NAR_Manuscript.docx (fresh, N1/N2/N4/N5/N8/N9/N10 fixes)
...
V28_ZIP = VERSION3_DIR / "CKI_NAR_Submission_v28.zip"
WORK_DIR = VERSION3_DIR / "CKI_NAR_Submission_v28"
V27_DIR = VERSION3_DIR / "CKI_NAR_Submission_v27"
...
def verify_n10_python_versions(work_dir):
    ...
    checks = {
        "Manuscript Data Availability": ("CKI_NAR_Manuscript_fulltext.txt", r"Python\s*≥\s*3\.9"),
        "Cover Letter": ("CKI_NAR_Cover_Letter_fulltext.txt", r"Python\s*\(\s*≥\s*3\.9\s*\)"),
    ...
    # Check no remaining "3.8+" anywhere in manuscript or cover letter
...
def build_v28():
    print("=" * 60)
    print("  CKI NAR Submission Package v28 Builder")
    print("  v27 + N10 Python Version Consistency")
    print("=" * 60)
    ...
    if WORK_DIR.exists():
        ...
        for f in _glob.glob(str(WORK_DIR / "*")):
            os.remove(f)
        os.rmdir(str(WORK_DIR))
    WORK_DIR.mkdir(parents=True)

    # Copy base files from v27 (figures, graphical abstract)
    # DOCX and fulltext files will be regenerated fresh
    if V27_DIR.exists():
        for f in V27_DIR.iterdir():
            if f.name.startswith("MANIFEST"):
                continue
            if f.name.endswith(".docx") or f.name.endswith("_fulltext.txt"):
                continue
            shutil.copy2(f, WORK_DIR / f.name)
        print(f"  Copied figures/GA from v27")
    elif (V26_DIR := VERSION3_DIR / "CKI_NAR_Submission_v26").exists():
        for f in V26_DIR.iterdir():
            ...
            if f.name.endswith(".docx") or f.name.endswith("_fulltext.txt"):
                continue
            shutil.copy2(f, WORK_DIR / f.name)
        print(f"  Copied figures/GA from v26 (v27 missing)")
    else:
        print(f"  ERROR: No source directory found!")
        return False

    # 1. Regenerate DOCX files fresh
    print(f"\n[1] Regenerating all DOCX files with v28 fixes...")

    # 1a. Main manuscript (N1/N2/N4/N5/N8/N9/N10 fixes)
    run_script(
        f'"{PYTHON}" -u generate_manuscript_nar.py',
        "Generating CKI_NAR_Manuscript.docx (N1/N2/N4/N5/N8/N9/N10 fixes)"
    )
    ...
    # 2. Write manifest
    print(f"\n[2] Writing MANIFEST_v28.txt...")
    manifest = f"""CKI NAR Submission Package v28
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}

Key Addition from v27: N10 Python Version Consistency

N10 Fix (New in v28 — resolved):
  - Manuscript Data Availability: "Python >=3.10" (was "3.8+", now matches Cover Letter)
  - Manuscript Methods: "Python 3.13.12" (actual runtime, unchanged)
  - Cover Letter: "Python (>=3.10)" (unchanged)
  - All 3 places now consistently state >=3.10 as minimum requirement
...
P1 Fixes (inherited from v26, v25 expert panel N5-N9):
  N5 — Reference renumbering (already covered above)
...
All v25 Issues (N1-N10) Resolved:
  P0: N1, N2, N3, N4 — Critical fixes
  P1: N6, N7, N8, N9 — Major fixes
  P2: N5, N10 — Reference renumbering, Python version consistency
...
Residual Model (Brain):
  30 Strong candidates, EVT-extrapolated P-values across 31,764 tests
...
Contents:
1. CKI_NAR_Manuscript.docx - Main manuscript (fresh, N1/N2/N4/N5/N8/N9/N10)
...
"""
    manifest_path = WORK_DIR / "MANIFEST_v28.txt"
    ...
    # 3. Create ZIP
    print(f"\n[3] Creating ZIP...")
    with zipfile.ZipFile(V28_ZIP, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(WORK_DIR):
            for fname in sorted(files):
                fpath = Path(root) / fname
                arcname = f"CKI_NAR_Submission_v28/{fname}"
                zf.write(fpath, arcname)

    # 4. Verify
    zip_size_mb = V28_ZIP.stat().st_size / (1024 * 1024)
    print(f"\n{'='*60}")
    print(f"  v28 Package Built")
    print(f"{'='*60}")
    print(f"ZIP: {V28_ZIP}")
    print(f"Size: {zip_size_mb:.1f} MB")

    with zipfile.ZipFile(V28_ZIP, "r") as zf:
