#!/usr/bin/env python3
"""Build CKI NAR Submission Package v25.

v25 incorporates 20 Major Issue fixes (M1-M20) from v20 expert panel review:
All fixes applied to source scripts in tasks #780-#784.

Major Issue Fixes Applied:
  M1 — Calibration caveat added to Discussion
  M2 — Method comparison caveat (SAMAP/CACIMAR/SATURN limitations)
  M4 — Cross-organ sample size caveat
  M5 — "Cohen's d" → "standardized effect size (SES)"
  M7 — Bootstrap null hypothesis clarification
  M9 — TCGA confounder discussion expansion
  M10 — Cell composition artifact discussion
  M11 — Brain migration evidence discussion
  M12 — Supplementary Section 3.9 TCGA caveats update
  M13 — Reproducibility Guide Section 5.5 Phase D items
  M14 — ω calibration factor rationale
  M15 — Supplementary Algorithm 1 correction
  M17 — Discussion section compression
  M19 — Reference numbering alignment
  M20 — k_n variability discussion (CV=97%)

Inherits from v24:
  - C1-C7 Critical fixes
  - Phase A-E expert panel fixes
  - Figures (1-6, S1-S7)
  - Graphical Abstract
  - Tables (Table1-2.docx)

Files regenerated:
  - CKI_NAR_Manuscript.docx (fresh, M1-M20 fixes)
  - CKI_NAR_Supplementary.docx (fresh, M12/M15 fixes)
  - CKI_NAR_Cover_Letter.docx (fresh)
  - CKI_NAR_Reproducibility_Guide.docx (fresh, M13 fixes)
"""
...
V25_ZIP = VERSION3_DIR / "CKI_NAR_Submission_v25.zip"
WORK_DIR = VERSION3_DIR / "CKI_NAR_Submission_v25"
V24_DIR = VERSION3_DIR / "CKI_NAR_Submission_v24"
...
def build_v25():
    print("=" * 60)
    print("  CKI NAR Submission Package v25 Builder")
    print("  20 Major Issues (M1-M20) Fixed")
    print("=" * 60)
    ...
    # Copy base files from v24 (figures, tables, graphical abstract)
    if V24_DIR.exists():
        for f in V24_DIR.iterdir():
        ...
        print(f"  Copied figures/tables/GA from v24")
    else:
        print(f"  ERROR: v24 directory not found at {V24_DIR}")
        return False

    # 1. Regenerate DOCX files fresh
    print(f"\n[1] Regenerating all DOCX files with M1-M20 fixes...")

    # 1a. Main manuscript (M1-M20 fixes)
    run_script(
        f'"{PYTHON}" -u generate_manuscript_nar.py',
        "Generating CKI_NAR_Manuscript.docx (M1-M20 fixes)"
    )
    ...
    # 1b. Supplementary materials (M12/M15 fixes)
    run_script(
        f'"{PYTHON}" -u notebooks/68_gen_supplementary_en.py',
        "Generating CKI_NAR_Supplementary.docx (M12/M15 fixes)"
    )
    ...
    # 1d. Reproducibility guide (M13 fixes)
    node_env = os.environ.copy()
    node_env["NODE_PATH"] = NODE_PATH
    run_script(
        f'"{NODE}" notebooks/100_gen_reproducibility_docx.js',
        "Generating CKI_NAR_Reproducibility_Guide.docx (M13 fixes)"
    )
    ...
    # 2. Write manifest
    print(f"\n[2] Writing MANIFEST_v25.txt...")
    manifest = f"""CKI NAR Submission Package v25
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}

20 Major Issue Fixes (M1-M20 from v20 expert panel review):

Batch 1 — Terminology & Quick Fixes (#780, #782):
  M5: "Cohen's d" → "standardized effect size (SES)" (all 4 docs)
  M7: Bootstrap null hypothesis clarified
  M10: Cell composition artifact discussion added
  M20: k_n variability discussion (CV=97%)

Batch 2 — Supplementary & Repro Guide (#781):
  M12: Supplementary Section 3.9 TCGA caveats updated
  M13: Reproducibility Guide Section 5.5 Phase D items
  M15: Supplementary Algorithm 1 correction

Batch 3 — Method/Discussion Caveats (#784):
  M2: Method comparison caveat (SAMAP/CACIMAR/SATURN limitations)
  M4: Cross-organ sample size caveat (n<5 flag)
  M9: TCGA confounder discussion expansion
  M11: Brain migration evidence discussion
  M14: ω calibration factor rationale

Batch 4 — Calibration & References (#783):
  M1: Calibration caveat added to Discussion
  M17: Discussion section compression
  M19: Reference numbering alignment

All Critical Issues (C1-C7) Resolved (inherited from v24):
  C1: BH-FDR q-value description (m=31,764)
...
Contents:
1. CKI_NAR_Manuscript.docx - Main manuscript (English, fresh, M1-M20)
...
5. Table1-2.docx - Standalone tables (unchanged)
...
"""
    manifest_path = WORK_DIR / "MANIFEST_v25.txt"
    ...
    # 3. Create ZIP
    print(f"\n[3] Creating ZIP...")
    with zipfile.ZipFile(V25_ZIP, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(WORK_DIR):
            for fname in sorted(files):
                fpath = Path(root) / fname
                arcname = f"CKI_NAR_Submission_v25/{fname}"
                zf.write(fpath, arcname)

    # 4. Verify
    zip_size_mb = V25_ZIP.stat().st_size / (1024 * 1024)
    print(f"\n{'='*60}")
    print(f"  v25 Package Built")
    print(f"{'='*60}")
    print(f"ZIP: {V25_ZIP}")
