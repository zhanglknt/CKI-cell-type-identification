#!/usr/bin/env python3
"""Build CKI Submission Package v42 (Genome Biology).
...
V38_ZIP = VERSION3_DIR / "CKI_Submission_v42.zip"
WORK_DIR = VERSION3_DIR / "CKI_Submission_v42"
...
def verify_v42_additions(v: Verifier):
    ...
    v.check(bool(re.search(r'283d65eb-2f53-46e9-a951-0da342e3d1f2', g)),
...
def verify_r2_additions(v: Verifier):
    ...
    s = v.supp_text()
    g = v.rg_text()
    mf = (v.wd / "MANIFEST_v42.txt").read_text(encoding="utf-8")
    import json as _json
...
def build_v38():
    print("=" * 60)
    print("  CKI Submission Package v42 (Genome Biology) Builder")
    ...
    # 3. Manifest
    print(f"\n[3] Writing MANIFEST_v42.txt...")
    manifest = f"""CKI Submission Package v42 (Genome Biology, Methodology article)
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Status: v42 = v41 + blind-review round-1 (four-expert panel,
2026-09-03, weighted 7.4/10) full fixes: all five P0 mechanical
items AND all five P1 substantive items resolved. Round-2 re-review
(8.0/6.5/9.0/9.0, mean 8.125): one P1 (Guide 5.7h / spot_check
human-pairs caliber unified to the phase35 4,851 analyzed pairs)
and all twelve round-2 P2 items resolved (Kang 'consistent with'
wording, Methods paragraph for the joint region-clustered
bootstrap, TF endpoint in the Discussion rule-matched sentence,
SN 5.1 pool clarification 43.7 vs 148.3, SN 5.2 Monte-Carlo note
at B = 200, severity summary LUAD exemption, nominal-P
annotations in SN 3.16 and Guide 5.7i, cross-organ composite
topic qualifier, per-pair P unified to 5.7e-4, notebook 82
uniform-draw reference key-path fix, JT implementation note,
MANIFEST SHA-256 checksums) (R2-1..R2-17).
...
"""
    ...
    with open(WORK_DIR / "MANIFEST_v42.txt", "w", encoding="utf-8") as f:
        f.write(manifest)
        ...
        for root, dirs, files in os.walk(WORK_DIR):
            for fn in sorted(files):
                if ZIP_EXCLUDE(fn):
                    continue
                fp = Path(root) / fn
                zf.write(fp, f"CKI_Submission_v42/{fn}")

    zip_mb = V38_ZIP.stat().st_size / (1024 * 1024)
    print(f"\n{'='*60}")
    print(f"  v42 (GB) Package: {V38_ZIP}")
    ...
    if v.failed == 0:
        print(f"\n  *** ALL {v.passed} CHECKS PASSED — v42 (GB) FINAL ***")
    else:
