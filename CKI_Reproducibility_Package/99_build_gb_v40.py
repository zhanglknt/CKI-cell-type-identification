#!/usr/bin/env python3
"""Build CKI Submission Package v40 (Genome Biology).
...
V38_ZIP = VERSION3_DIR / "CKI_Submission_v40.zip"
WORK_DIR = VERSION3_DIR / "CKI_Submission_v40"
...
FIGURE_MAP = {
    ...
    "Supplementary_Figure_S7": "Supplementary_Figure_S7",
...
def verify_v40_additions(v: Verifier):
    ...
    v.check(bool(re.search(r'Cochran-Armitage trend z = 61', t)),
            "V40-5 Cochran-Armitage trend z = 61 in MS")
    v.check(bool(re.search(r'9\.6 \u00d7 10\u207b\u00b3\u00b9', t)),
            "V40-6 Strong-tier hypergeometric P = 9.6e-31 in MS")
    v.check(bool(re.search(r'6 of 10 candidates versus a 19\.4% base rate', t)),
            "V40-7 thalamic-relay enrichment (6/10 vs 19.4%) in MS")
    v.check(bool(re.search(r'9 of 10 versus 22\.7%', t)),
            "V40-8 combined thalamo-temporal enrichment (9/10 vs 22.7%) in MS")
    ...
    v.check(bool(re.search(r'attenuates by only 6% pooled', t)),
            "V40-11 TCGA composition attenuation 6% pooled in MS")
    v.check(bool(re.search(r'2\u201339% per cancer type', t)),
            "V40-12 TCGA composition attenuation range 2-39% in MS")
    v.check(bool(re.search(r'Spearman \u03c1 = 0\.21\u20130\.46', t)),
            "V40-13 TCGA composition Spearman range 0.21-0.46 in MS")
...
def verify_legacy(v: Verifier):
    ...
    # N5b: every reference 1..49 is cited in the body (ranges expanded)
    cited = set()
    for m2 in re.finditer(r'\[(\d{1,2}(?:\s*,\s*\d{1,2})*(?:\s*[\u2013-]\s*\d{1,2})?)(\s*;[^\]]*)?\]', body):
        ...
        try:
            ...
            vals = []
        if vals and all(1 <= x <= 49 for x in vals):
            if len(vals) == 2 and ('\u2013' in m2.group(1) or '-' in m2.group(1)):
                cited.update(range(vals[0], vals[1] + 1))
            else:
                cited.update(vals)
    v.check(len(cited) == 49, f"N5b All 49 refs cited in body ({len(cited)}/49)")
...
def verify_p2_e4(v: Verifier):
    ...
    # GB/Vancouver: reference list must be numbered 1..49 matching [n] citations
    ref_nums = [int(n) for n in re.findall(r'^(\d{1,2})\.\s+\S', ref_section, re.M)]
    v.check(ref_nums == list(range(1, 50)),
            f"E4-8 Reference list numbered 1..49 ({len(ref_nums)} entries, "
            f"sequence {'OK' if ref_nums == list(range(1, 50)) else 'BROKEN'})")
...
def verify_v38_reviewer_fixes(v: Verifier):
    ...
    v.check(bool(re.search(r'v0\.4\.3|0\.4\.3', t)) and bool(re.search(r'v0\.4\.3|0\.4\.3', rg))
            and not re.search(r'0\.4\.2', t) and not re.search(r'0\.4\.2', rg),
            "V38-3g version 0.4.3 in MS + guide (no 0.4.2 residue)")
    _pkg_init = (BASE_DIR / "cki" / "__init__.py").read_text(encoding="utf-8")
    _pyproject = (BASE_DIR / "pyproject.toml").read_text(encoding="utf-8")
    v.check('__version__ = "0.4.3"' in _pkg_init
            and re.search(r'^version\s*=\s*"0\.4\.3"', _pyproject, re.M) is not None,
            "REL-1 package version bumped to 0.4.3 in cki/__init__.py + pyproject.toml")
...
def build_v38():
    print("=" * 60)
    print("  CKI Submission Package v40 (Genome Biology) Builder")
    ...
    # 3. Manifest
    print(f"\n[3] Writing MANIFEST_v40.txt...")
    manifest = f"""CKI Submission Package v40 (Genome Biology, Methodology article)
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Status: v40 = v39 + three pre-submission additions (set-level brain
coherence checks, TCGA composition sensitivity check, when-to-use
decision guide); all v39 content otherwise unchanged.
...
=== v38 Changes (v37 peer-review fixes) ===
  ...
  - R5-C5 Dockerfile (python:3.14-slim) in repository; R5-M1 scanpy
    ...
    all at v0.4.3, tagged and released on GitHub (v0.4.3) and archived
    on Zenodo (concept DOI 10.5281/zenodo.20405458).
...
=== v39 Changes (Genome Biology conversion, no content change) ===
  ...
  - References renumbered by first appearance in GB layout
    ...
    was found; reference list now explicitly numbered 1..49 (checked by
    E4-8). Reference order and content are otherwise identical to v38.
...
"""
    ...
    print(f"\n{'='*60}")
    print(f"  v40 (GB) Package: {V38_ZIP}")
    ...
    if v.failed == 0:
        print(f"\n  *** ALL {v.passed} CHECKS PASSED — v40 (GB) FINAL ***")
    else:
