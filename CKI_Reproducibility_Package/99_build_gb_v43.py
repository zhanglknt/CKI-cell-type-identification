#!/usr/bin/env python3
"""Build CKI Submission Package v43 (Genome Biology).
...
V38_ZIP = VERSION3_DIR / "CKI_Submission_v43.zip"
WORK_DIR = VERSION3_DIR / "CKI_Submission_v43"
...
def verify_v42_additions(v: Verifier):
    ...
    v.check(ref_nums == list(range(1, 56)),
            "V42-5 reference list numbered 1..55 intact after renumbering")

    # ---- P0-2: Bergmann-glia region-clustered CI unified ----
    v.check(len(re.findall(r'8\.49, 19\.52', t)) == 3,
            "V42-6 MS region-clustered CI [8.49, 19.52] 3x (Results + Discussion x2)")
    ...
    v.check(bool(re.search(r'sensitive to bootstrap resampling', t)),
            "V42-10 MS seed-sensitivity note (7 regions)")
...
def verify_p1_additions(v: Verifier):
    ...
    v.check(bool(re.search(r'roughly two of 37 tests would cross raw P < 0\.05', t)),
            "V42-35 MS Kang multiplicity disclosure (2/37 expected)")
    ...
    # ---- P1-3: k_f-only ordering controls (notebook 83) ----
    v.check(bool(re.search(r'predominantly baseline \(k_n\) effects rather than functional-divergence orderings', t)),
            "V42-48 MS severity topic sentence hedged (denominator-dominated)")
    v.check(bool(re.search(r'mean k_f rises from 0\.50 in G1 to 0\.53 in G4', t)),
            "V42-49 MS LIHC k_f reversal (0.50 -> 0.53)")
    v.check(bool(re.search(r'Luminal A lowest mean k_f, 0\.479; Basal-like highest, 0\.536', t)),
            "V42-50 MS BRCA PAM50 k_f-only reversal")
    v.check(bool(re.search(r'persists under k_f-only \(Kruskal-Wallis P = 0\.015\)', t)),
            "V42-51 MS LUAD mutation contrast persists in k_f (P = 0.015)")
    v.check(bool(re.search(r'r = 0\.23, P = 0\.37', t)),
            "V42-52 MS cross-organ omega-vs-k_f Spearman r = 0.23")
    v.check(bool(re.search(r'r = 0\.10 among the five well-sampled types', t)),
            "V42-53 MS well-sampled k_f concordance r = 0.10")
    ...
    v.check(bool(re.search(r'all 12 published strata', s)),
            "V42-61 SN 3.16 TCGA 12-strata reproduction")
...
def verify_r2_additions(v: Verifier):
    ...
    s = v.supp_text()
    g = v.rg_text()
    mf = (v.wd / "MANIFEST_v43.txt").read_text(encoding="utf-8")
    import json as _json
    ...
    v.check("providing evidence consistent with" in t,
            "R2-3 MS Kang results sentence uses 'consistent with'")
    ...
    # ---- E2 P2-1: severity summary hedges LUAD exception ----
    v.check("predominantly (rather than exclusively) denominator-dominated" in t
            and "is the one exception" in t,
    ...
    v.check("under the composite \u03c9 ordering, of which only the extremes reproduce under k_f" in t,
            "R2-12 MS cross-organ topic sentence composite qualifier")
...
def verify_v43_reprofix(v: Verifier):
    ...
    v.check("v0.4.6" in cl, "V43-15 cover letter release tag v0.4.6")
    ...
    v.check("cki:0.4.6" in _readme and "cki:0.4.4" not in _readme,
            "V43-17 README docker tag 0.4.6")
    v.check("releases/tag/v0.4.6" in _data_readme,
            "V43-18 data README release link v0.4.6")
...
def verify_v38_reviewer_fixes(v: Verifier):
    ...
    v.check(bool(re.search(r'v0\.4\.6|0\.4\.6', t)) and bool(re.search(r'v0\.4\.6|0\.4\.6', rg))
            and not re.search(r'0\.4\.[2345]', t) and not re.search(r'0\.4\.[2345]', rg),
            "V38-3g version 0.4.6 in MS + guide (no 0.4.2-0.4.5 residue)")
    _pkg_init = (BASE_DIR / "cki" / "__init__.py").read_text(encoding="utf-8")
    _pyproject = (BASE_DIR / "pyproject.toml").read_text(encoding="utf-8")
    v.check('__version__ = "0.4.6"' in _pkg_init
            and re.search(r'^version\s*=\s*"0\.4\.6"', _pyproject, re.M) is not None,
            "REL-1 package version bumped to 0.4.6 in cki/__init__.py + pyproject.toml")
...
def verify_v38_text_revisions(v: Verifier):
    ...
    # --- terminology ---
    # T4 updated for v43: the repro guide now states the softmax
    # normalization step (repro-review-v3 P1 fix), so 'softmax' stays
    # banned only in supplementary/cover letter.
    v.check(not re.search(r'softmax', s + cl, re.I),
            "T4 'softmax' confined to the manuscript + guide normalization step")
...
def build_v38():
    print("=" * 60)
    print("  CKI Submission Package v43 (Genome Biology) Builder")
    print("  Blind-review round-1 P0 fixes + fresh DOCX rebuild")
    print("=" * 60)
    ...
    # 3. Manifest
    print(f"\n[3] Writing MANIFEST_v43.txt...")
    manifest = f"""CKI Submission Package v43 (Genome Biology, Methodology article)
Built: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Status: v43 = v42 + repro-review-v3 fixes (2026-09-04; three-verifier
read-only audit of the Reproducibility Guide against source code and
results files): dead CELLxGENE collection ID replaced (P0), dead
Siletti repo / probeMap URLs fixed (P1), softmax normalization and
positivity-guard wording corrected in the Guide, seed-20260903
exceptions disclosed, cross-species matching note corrected, Strong
tier attributed to 08d, version bumped to 0.4.6 (V43-1..V43-20).
Package released as tag v0.4.6.
...
"""
    ...
    with open(WORK_DIR / "MANIFEST_v43.txt", "w", encoding="utf-8") as f:
        f.write(manifest)
        ...
        for root, dirs, files in os.walk(WORK_DIR):
            for fn in sorted(files):
                if ZIP_EXCLUDE(fn):
                    continue
                fp = Path(root) / fn
                zf.write(fp, f"CKI_Submission_v43/{fn}")

    zip_mb = V38_ZIP.stat().st_size / (1024 * 1024)
    print(f"\n{'='*60}")
    print(f"  v43 (GB) Package: {V38_ZIP}")
    ...
    if v.failed == 0:
        print(f"\n  *** ALL {v.passed} CHECKS PASSED — v43 (GB) FINAL ***")
    else:
