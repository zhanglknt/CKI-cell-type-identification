...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      p("Version: 0.4.4 (editable install from project root)"),
      ...
      p("Cross-species matching note (Supplementary Fig. S10): the mouse-human cell-type matching used for the cross-species comparison is case-insensitive with prefix matching (notebooks/_ed_fig2_clean.py), because cell-type names in results/full_matrix_pairs.csv are truncated to 20 characters. This rule matches 15 shared cell types between the Tabula Muris and Tabula Sapiens annotations; exact-string matching would match none. Re-running the comparison with a different matching rule will change the plotted cell-type set."),
      ...
      p("      NOTE: two of the three CSV files below are shipped as PRE-FIX outputs and"),
      p("      are SUPERSEDED. (1) human_bootstrap_results.csv was produced by the broken"),
      p("      legacy 08b_human_bootstrap_csv.py, which bootstrapped pre-computed omega"),
      p("      values and yielded uninformative P-values near 0.5 for every group;"),
      p("      regenerate it by re-running 08b_human_bootstrap_v2.py (cell-level label"),
      p("      permutation, which overwrites the same file). (2) brain_bootstrap_results.csv"),
      p("      holds pre-fix cell-type values (e.g. astrocyte omega 83.64, OPC 40.83) that"),
      p("      contradict the manuscript; all authoritative brain statistics are in"),
      p("      brain_bs_null_ct_test.csv (astrocyte 82.75, OPC 40.62) and the other"),
      p("      brain_bs_null_* files from the 08d/08e block-shuffle null (Section 5.3c)."),
      code("      results/tcga_bootstrap_results.csv           # TCGA permutation P-values (B=1000)"),
      code("      results/human_bootstrap_results.csv          # Human permutation P-values (B=1000) [SUPERSEDED - pre-fix output; regenerate with 08b_human_bootstrap_v2.py]"),
      code("      results/brain_bootstrap_results.csv          # Brain permutation P-values (B=1000) [SUPERSEDED - pre-fix values; authoritative brain stats in brain_bs_null_ct_test.csv]"),
      p(""),
      ...
      p("[\u2713] Install CKI v0.4.4: pip install -e ."),
      ...
      p("[\u2713] Note: Benjamini-Hochberg FDR correction is applied within each dataset. BH-adjusted P-values are available in tcga_bootstrap_results.csv (and in human_bootstrap_results.csv once it is regenerated with 08b_human_bootstrap_v2.py); for the brain use brain_bs_null_ct_test.csv (cell-type level) and brain_bs_null_results.csv (q_fdr, per pair) instead of the superseded brain_bootstrap_results.csv. (Section 5.2)"),
