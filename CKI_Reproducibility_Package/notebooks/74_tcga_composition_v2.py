# -*- coding: utf-8 -*-
"""
TCGA composition check v2: myeloid panel + sample-level cluster bootstrap.

Motivation (v40 review P2-6 / E2-M3 and P2-9 / E1-M5):
  (a) The original three-panel marker set (immune=lymphoid only, stromal,
      epithelial) omitted myeloid markers (CD68/CD163/LST1/FCGR3A/C1QA),
      although tumor-associated macrophages are the dominant infiltrate in
      most TCGA bulk tumors -> composition adjustment was incomplete and
      the "6% pooled attenuation" overstated the "composition does not
      explain the k_n reversal" conclusion.
  (b) The 25,306 pairs are built from ~3,596 samples; every sample enters
      thousands of pairs, so the OLS P-values / R^2 under independence
      are invalid. We re-estimate the attenuation with a sample-level
      cluster bootstrap and report interval estimates.

Design:
  - Pair table is REUSED from results/tcga_composition_pairs.csv (the v2
    pipeline, seed 42, is deterministic; per-pair k_n/k_f/omega unchanged).
  - A single pass over the TCGA matrix extracts raw TPM for all panel genes
    (original 3 panels + myeloid) for all target samples. Per cancer, panel
    genes are filtered exactly as in 73_tcga_composition_check.py
    (mean TPM >= 0.5 within the cancer), log2(TPM+1)-transformed, z-scored
    within the cancer, and |Delta| is attached to each pair.
  - Primary model: log k_n ~ d_immune + d_myeloid + d_stromal + d_epithelial
    + TT  (4-panel adjustment), per cancer and pooled; reported alongside the
    original 3-panel attenuation for direct comparison.
  - Cluster bootstrap (B=200, seed 42): within each cancer, resample tumor
    and normal samples with replacement; keep pairs from the existing pair
    table whose two members were both drawn, weighted by the product of
    draw multiplicities; run weighted OLS per iteration. Report the 2.5/97.5
    percentile interval of the attenuation (1 - beta_TT_full/beta_TT_base).

Outputs:
  results/tcga_composition_v2.csv   (test-level results)
  results/tcga_composition_v2.txt   (summary)
"""
import sys, os
...
PAIRS_FILE = Path(r"C:\Users\KnightZ\Desktop\细胞受选择\results\tcga_composition_pairs.csv")
...
# ====================================================================
# 1. Load pair table (deterministic v2 pipeline output, seed 42)
# ====================================================================
print("1. Loading pair table...")
pdf = pd.read_csv(PAIRS_FILE)
print(f"  {len(pdf)} pairs "
      f"(NN={len(pdf[pdf.pair_type=='NN'])}, TT={len(pdf[pdf.pair_type=='TT'])})")

# ====================================================================
# 2. Single pass over TCGA matrix: raw TPM for panel genes, all samples
#    of the 5 target projects (01/11)
# ====================================================================
...
target_cols = []          # (sample_id, project, code, column index)
...
# store raw TPM: dict[(proj, code)][gene_symbol][sample_id] = value
raw = {}
n_panel_rows = 0
with gzip.open(TCGA_FILE, "rt") as fh:
    fh.readline()
        ...
        for (sid, proj, code, ci) in target_cols:
            if ci < len(parts):
                try:
                    ...
                    v = 0.0
                # Xena-style matrices contain small negative values and
                # 'nan' strings; clamp exactly as the authoritative 73
                # pipeline does (np.maximum(expr, 0))
                if v != v or v < 0.0:
                    v = 0.0
            else:
                v = 0.0
...
print(f"  panel gene rows captured: {n_panel_rows} "
      f"(unique symbols: {len(set(s for d in raw.values() for s in d))}) "
      f"({time.time()-t0:.0f}s)")

# ====================================================================
# 3. Per-cancer z-scored panel scores (identical rule to 73:
#    mean TPM >= 0.5 filter within cancer -> log2(TPM+1) -> z-score)
# ====================================================================
print("3. Per-cancer panel scores...")
comp_scores = {}   # comp_scores[(cancer, panel)][sample_id] = z
gene_report = {}   # gene_report[cancer][panel] = list of genes kept
for cancer in TARGET_PROJECTS:
    ...
    for pname, symbols in PANELS.items():
        # gather values per gene over this cancer's samples
        gene_vals = {}
        for sym in symbols:
            if sym not in raw.get((cancer, "01"), {}) and \
               sym not in raw.get((cancer, "11"), {}):
                continue
            # merge the two sample codes (a gene row was stored once per code key);
            # a symbol may map to several Ensembl rows -> average them (as in 73)
            d_acc = {}
...
# ====================================================================
# 4. Attach myeloid deltas to pair table
# ====================================================================
def dz(cancer, pname, sid):
    return comp_scores.get((cancer, pname, sid), 0.0)

pdf["d_myeloid"] = [
    abs(dz(r.cancer, "myeloid", r.sample_a) - dz(r.cancer, "myeloid", r.sample_b))
...
# ====================================================================
# 6. Sample-level cluster bootstrap (B=200)
# ====================================================================
print(f"4. Cluster bootstrap by sample (B={N_BOOT}, seed {RANDOM_SEED})...")
rng = np.random.default_rng(RANDOM_SEED)

# per cancer, index pairs for fast subsetting
by_cancer = {}
for cancer in TARGET_PROJECTS:
...
def boot_attenuation(panels):
    """One bootstrap iteration -> (att_pooled, {cancer: att}), or None."""
    ys, Xs, tts, ws = [], [], [], []
    atts = {}
    for cancer in TARGET_PROJECTS:
        ...
        # keep pairs whose both members were drawn; weight = mult_a * mult_b
        sub_frames = []
...
while n_ok < N_BOOT:
    ...
    if r4 is None:
        continue
    r3 = boot_attenuation(P3)   # uses same rng stream; separate draw
    if r3 is None:
        continue
    a4, by4 = r4
    a3, _ = r3
...
out.to_csv(RESULTS_DIR / "tcga_composition_v2.csv", index=False)

lines = ["TCGA composition check v2: myeloid panel + cluster bootstrap", "=" * 70,
         f"Pairs: {len(pdf)} (reused from 73, seed 42 pipeline); "
...
for tid, desc, eff, p in tests:
    ...
    lines.append("")
(RESULTS_DIR / "tcga_composition_v2.txt").write_text("\n".join(lines), encoding="utf-8")

print("\n".join(lines))
print(f"\nSaved: {RESULTS_DIR/'tcga_composition_v2.csv'}")
print(f"Saved: {RESULTS_DIR/'tcga_composition_v2.txt'}")
