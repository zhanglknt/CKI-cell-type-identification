...
p(
    ...
    'identified 30 cell-type-specific developmental signatures spanning four biological mechanisms (two reaching statistical significance) among 31,764 comparisons. '
...
p(f'Step 3: \u03c9 = k_f/k_n. For statistical inference, we perform bootstrap permutation testing (B = 1,000 for all datasets: mouse, human, TCGA, and brain). Cell labels are randomly shuffled and \u03c9 recalculated to generate a null distribution. The empirical P-value is P = (count(\u03c9_null \u2265 \u03c9_obs) + 1)/(B + 1), and effect size is reported as standardized effect size (SES = (\u03c9_obs \u2212 \u03bc_null) / \u03c3_null). Benjamini-Hochberg FDR correction is applied within each dataset. For the larger-scale analyses (Tabula Sapiens, TCGA, brain atlas), non-parametric statistical tests and descriptive statistics (median, IQR, effect sizes) are also reported.')
...
p(f'Among {_br["total_pairs"]:,} cross-region comparisons, {_br["n_strong"]} ({_br["pct_strong"]:.2f}%) were classified as Strong migration candidates: Astrocyte (6), fibroblast (1), microglia (10), oligodendrocyte (10), and vascular cells (3). Another {_br["n_moderate"]:,} pairs ({_br["pct_moderate"]:.2f}%) were Moderate candidates, and {_br["n_weak"]:,} ({_br["pct_weak"]:.2f}%) were Weak candidates. Permutation testing (B = 10,000) revealed that 16 of 30 Strong signals reached the permutation P-value floor (P = 9.99 \u00d7 10\u207b\u2075, i.e., no permutation out of 10,000 produced a residual as extreme as observed): all 6 astrocyte and all 10 oligodendrocyte signals. The remaining 14 signals (10 microglia, 1 fibroblast, 3 vascular) showed no evidence of deviation from the multiplicative null model (all P \u2265 0.76). Given the large number of tests (m = 31,764) and P-value floor saturation (36.3% of signals at P = 9.99 \u00d7 10\u207b\u2075), formal FDR correction is not applicable; the 16 floor-reaching signals are interpreted as descriptive evidence of residual deviation from the multiplicative null, not as FDR-controlled discoveries. This indicates that the astrocyte and oligodendrocyte signals reflect robust biological patterns, whereas the microglia, fibroblast, and vascular signals may represent stochastic variation within the threshold criteria.')
