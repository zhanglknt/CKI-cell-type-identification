...
p(
    ...
    'identified 30 cell-type developmental signatures spanning four biological mechanisms (two with permutation support) among 31,764 comparisons. '
...
p(f'The calibration confirmed correct baseline behavior. We performed six control comparisons in which we randomly split the same cell population into two halves. The mean \u03c9 was {_mc["control_mean"]:.2f} (median {_mc["control_median"]:.2f}, range {_mc["control_min"]:.2f}\u2013{_mc["control_max"]:.2f}, CV \u2248 52%), indicating substantial variability in the empirical baseline estimate. None of the six comparisons reached statistical significance (all P > 0.05, one-sided bootstrap test). This confirms that CKI recognizes biologically equivalent cell populations as having no significant functional divergence. We note that while these results are consistent with baseline behavior, formal equivalence testing (e.g., two one-sided tests, TOST) with a larger calibration sample would provide stronger statistical evidence for equivalence; the current sample size (n = 6) with a coefficient of variation of ~52% limits the power of such tests and the precision of the calibration factor (\u03c9_cal = \u03c9 / 6.67).')
