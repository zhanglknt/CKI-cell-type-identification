...
_refs_nar = [
    ...
    'W\u00e4lchli,T., Ghobrial,M., Schwab,M.E., Takada,S., Zhong,H., Le,J. et al. (2024) Single-cell atlas of the human brain vasculature across development, adulthood and disease. *Nature*, **632**, 603\u2013613.',
...
p(
    ...
    'metrics conflate neutral variation with functional adaptation. Inspired by the Ka/Ks ratio '
    'in molecular evolution, we present CKI (Cell-state Kinetic Index), which decomposes '
    'transcriptomic divergence into a neutral offset rate (k_n, from housekeeping gene expression) '
    'and a functional conversion rate (k_f, from identity gene expression), with \u03c9 = k_f/k_n '
    'quantifying selective remodeling. We validated CKI across four datasets: the Tabula Muris '
    'mouse atlas, Tabula Sapiens human atlas, TCGA pan-cancer data, and a human brain '
    'single-nucleus atlas totaling millions of cells. Calibration confirmed neutral behavior '
    f'for equivalent populations (mean \u03c9 = {_mc["control_mean"]:.2f}, all P > 0.05). CKI \u03c9 '
    f'was negatively correlated with all standard distance metrics (Spearman r = \u2212{abs(_sc["max"]):.2f} '
    f'to \u2212{abs(_sc["min"]):.2f}, all P < 0.001), proving it captures an independent information '
    'dimension. Cancer analysis revealed transcriptional convergence across genetically diverse '
    'tumors. Brain regional analysis identified 30 cell-type-specific developmental origin '
    'signatures among 31,764 comparisons. CKI provides a principled framework for quantifying '
    'selective transcriptomic remodeling, available as an open-source Python package.'
)
...
# ============================================================
# Extended Data Figure legends (renamed for NAR convention)
# ============================================================
...
p('Supplementary Figure S2. Cross-species validation details. (a) Cross-species \u03c9 conservation: scatter plot of human vs. mouse \u03c9 for shared cell types, with Spearman r and P-value. (b) HK gene set detection stability: overlap between human and mouse HK gene sets detected by the combined criterion. (c) \u03c9 distribution comparison between mouse (n = 15 shared cell types) and human (n = 4,851 pairs).')

p('Supplementary Figure S3. TCGA per-cancer matrices. Pairwise \u03c9 matrices for six cancer types (BRCA, KIRC, LIHC, LUAD, COAD, HNSC) showing tissue-level transcriptomic divergence structure within each cancer cohort.')
