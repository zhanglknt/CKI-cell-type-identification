...
p(
    ...
    f'baseline behavior for equivalent populations (mean \u03c9 = {_mc["control_mean"]:.2f}, '
    f'all P > 0.05). CKI \u03c9 was negatively correlated with all standard distance metrics '
    ...
    'identified 30 cell-type-specific developmental origin signatures among 31,764 comparisons. '
...
p('We realized that this question mirrors a problem addressed in molecular evolution. When comparing two DNA sequences, the Ka/Ks ratio (also called dN/dS) distinguishes nonsynonymous changes (Ka, which alter the protein) from synonymous changes (Ks, which are largely silent). Ka/Ks uses synonymous sites as an internal baseline: the same mutational process produces both types of change, allowing the ratio to reveal selection. While CKI does not share Ka/Ks\u2019s formal mathematical properties (notably the shared mutation rate that cancels in the ratio; see Discussion), it adopts an analogous heuristic logic for transcriptomic comparisons.')
...
p(f'The cross-organ \u03c9 ranking reveals a broad spectrum of conservation across {len(t2)} cell types (Table 2). {t2[0][0]}s (mean \u03c9 = {t2[0][1]}, n = {t2[0][3]}) and {t2[1][0]}s (mean \u03c9 = {t2[1][1]}, n = {t2[1][3]}) were among the most conserved, followed by {t2[2][0]}s (mean {t2[2][1]} \u00b1 {t2[2][2]}, n = {t2[2][3]}) and {t2[3][0]}s (mean {t2[3][1]} \u00b1 {t2[3][2]}, n = {t2[3][3]}). {mac[0]}s, the most abundant cell type in cross-organ comparisons (n = {mac[3]}), showed intermediate conservation (mean {mac[1]} \u00b1 {mac[2]}). At the divergent end of the spectrum, {last2[0][0]}s (mean {last2[0][1]} \u00b1 {last2[0][2]}, n = {last2[0][3]}) and {last2[1][0]}s (mean {last2[1][1]} \u00b1 {last2[1][2]}, n = {last2[1][3]}) were the most organ-specific cell types. Endothelial cells are known to express organ-specific gene programs tailored to local vascular needs (32). We note that several cell types (particularly those with n = 1 or 3) have small sample sizes, and their rankings should be interpreted with appropriate caution.')
