...
n_genes = 1000
# Raw counts
raw = np.random.poisson(1.0, n_genes)
# normalize_total
raw_sum = raw.sum()
...
# So log1p values are non-negative! ensure_probability_distribution will NOT truncate anything.
...
pb1_log = np.log1p(np.random.poisson(2.0, n_genes) / np.random.poisson(2.0, 1)[0] * 1e4)
pb2_log = np.log1p(np.random.poisson(1.5, n_genes) / np.random.poisson(1.5, 1)[0] * 1e4)

# HK indices (first 50 genes)
hk_indices = list(range(50))

# Method A: pass log1p values to compute_kn (as CKI docs suggest)
kn_a = compute_kn(pb1_log, pb2_log, hk_indices)
print(f"  k_n (log1p input) = {kn_a:.6f}")

# Method B: pass normalized values to compute_kn (not log1p)
pb1_norm = np.random.poisson(2.0, n_genes) / np.random.poisson(2.0, 1)[0] * 1e4
pb2_norm = np.random.poisson(1.5, n_genes) / np.random.poisson(1.5, 1)[0] * 1e4)
...
# Create a minimal adata with 20 fake cells (10 per group)
n_cells = 20
...
# Normalize + log1p (as CKI docs say)
...
# Extract the same pseudobulks that compute() would compute internally
pb_ct1 = adata_test[adata_test.obs['cell_type'] == 'CT1'].X.mean(axis=0)
pb_ct2 = adata_test[adata_test.obs['cell_type'] == 'CT2'].X.mean(axis=0)
# pb_ct1, pb_ct2 are already log1p transformed (because adata_test is log1p transformed)
...
print("  - This is CKI's design choice (not 'bug'), as documented.")
