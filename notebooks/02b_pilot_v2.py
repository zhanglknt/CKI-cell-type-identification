"""
CKI Mouse Pilot v2: Re-Validation with CKI v0.2.0
==================================================
UPDATES:
...
print("\n" + "=" * 60)
print("4. Gene index mapping (global HK only, k_f will be per-pair DE)...")
print("=" * 60)

gene_names = adata.var_names.tolist()
hk_indices = np.array([i for i, g in enumerate(gene_names) if g in hk_mouse_genes])
print(f"  Global HK genes: {len(hk_indices)}")
...
ct_all_cells = {}  # key: (tissue, ct) -> all cells expression
ct_pb_largest = {}  # largest mouse group pseudobulk
ct_cells_largest = {}  # largest mouse group cells
...
# C: Control (same tissue, same CT, random split)
control_pairs = [
    ("Liver", "hepatocyte"),
...
# ===== 7. Run CKI v0.2.0 (per-pair top-200 DE k_f) =====
print("\n" + "=" * 60)
...
for comp in comparisons:
    ...
    non_hk_mask = np.ones(len(pb_a), dtype=bool)
    ...
    top_global = non_hk_global_idx[top_local]
    ...
    for b in range(N_BOOTSTRAP):
        ...
        non_hk_abs_null = abs_diff_null[non_hk_mask]
        top_local_null = np.argpartition(non_hk_abs_null, -top_n)[-top_n:]
        top_local_null = top_local_null[np.argsort(non_hk_abs_null[top_local_null])[::-1]]
        top_global_null = non_hk_global_idx[top_local_null]
...
for cat in ["C_control", "S_same_ct", "D_diff_ct", "X_cross"]:
    ...
    if len(subset) > 0:
        print(f"  {cat}: n={len(subset)}, mean_omega={subset['omega'].mean():.2f}, "
              ...
              f"range=[{subset['omega'].min():.2f}, {subset['omega'].max():.2f}]")
...
results_csv = RESULTS_DIR / "mouse_pilot_v2_results.csv"
results_df.to_csv(results_csv, index=False)
print(f"  Saved: {results_csv}")

# Also save key values for manuscript
key_vals = {}
for cat in ["C_control", "S_same_ct", "D_diff_ct", "X_cross"]:
    subset = results_df[results_df["category"] == cat]
    if len(subset) > 0:
...
if len(ctrl) > 0:
    ...
    key_vals["control_min"] = round(ctrl["omega"].min(), 2)
    key_vals["control_max"] = round(ctrl["omega"].max(), 2)

kv_csv = RESULTS_DIR / "mouse_pilot_v2_key_values.csv"
pd.DataFrame([key_vals]).to_csv(kv_csv, index=False)
print(f"  Saved: {kv_csv}")

print("\n" + "=" * 60)
print("Pilot v2 analysis complete!")
print("=" * 60)
