# -*- coding: utf-8 -*-
"""
nc52 (v52 revision, reviewer B1): ex-CC default for the four-panel marker
composition regression.

Line-for-line mirror of notebooks/86_tcga_composition_linear_norm_v44.py
(four-panel marker set incl. myeloid, per-cancer z-scored panel scores,
log(k_n) OLS attenuation, sample-level cluster bootstrap B=1000 seed 42,
Spearman rho of k_n vs four-panel |Delta composition|) with ONE change: the
32 ILSBio CC tumours are excluded by default — both from the pair set (every
pair touching a CC sample dropped) and from the per-cancer z-score
standardization population (LIHC panel scores are standardized on ex-CC
samples only).

Outputs (NEW files, nc52 prefix):
  results/nc52_tcga_composition_excc.csv
  results/nc52_tcga_composition_excc.txt
Seed: 42. B=1000.
"""
import sys, os
...
# ====================================================================
# 1. Load v44 pair table (TT + NN only), ex-CC default
# ====================================================================
print("1. Loading v44 pair table (ex-CC default)...")
pdf = pd.read_csv(PAIRS_FILE)
pdf = pdf[pdf["pair_type"].isin(["TT", "NN"])].reset_index(drop=True)
n_cc = int((pdf.sample_a.map(is_cc) | pdf.sample_b.map(is_cc)).sum())
pdf = pdf[~(pdf.sample_a.map(is_cc) | pdf.sample_b.map(is_cc))].reset_index(drop=True)
n_kn_nonpos = int((pdf["kn"] <= 0).sum())
pdf = pdf[pdf["kn"] > 0].reset_index(drop=True)
print(f"  {len(pdf)} pairs after dropping {n_cc} CC-touching pairs "
      f"(NN={len(pdf[pdf.pair_type=='NN'])}, TT={len(pdf[pdf.pair_type=='TT'])}); "
      f"{n_kn_nonpos} pairs with k_n<=0 excluded")

# ====================================================================
# 2. Single pass over TCGA matrix: raw TPM for panel genes (verbatim 86)
# ====================================================================
...
for k, sid in enumerate(header_line[1:], 1):
    ...
    if proj in TARGET_PROJECTS and code in ("01", "11"):
        if is_cc(sid):
            continue  # ex-CC default: CC samples excluded from standardization
        target_cols.append((sid, proj, code, k))
print(f"  target samples in matrix (ex-CC): {len(target_cols)}")
...
# ====================================================================
# 3. Per-cancer z-scored panel scores (identical rule to 86; ex-CC
#    standardization population)
# ====================================================================
print("3. Per-cancer panel scores (ex-CC standardization)...")
comp_scores = {}
gene_report = {}
...
# ====================================================================
# 4. Attach all four panel deltas to the ex-CC pair table
# ====================================================================
...
# ====================================================================
# 5. Weighted OLS helpers (verbatim 86)
# ====================================================================
...
# ====================================================================
# 6. Sample-level cluster bootstrap (B=1000, seed 42; verbatim 86 scheme)
# ====================================================================
...
out.to_csv(RESULTS_DIR / "nc52_tcga_composition_excc.csv", index=False)

out_lines = ["nc52 TCGA composition check, ex-CC default (linear normalization): "
             "myeloid panel + cluster bootstrap",
             "=" * 78,
             f"Pairs: {len(pdf)} ex-CC (v44 linear-norm pair table; {n_cc} CC-touching "
             f"pairs dropped; {n_kn_nonpos} pairs with k_n<=0 excluded); CC samples "
             f"also excluded from the z-score standardization population; "
             f"panels: immune(7) + myeloid(5) + stromal(6) + epithelial(4)", ""]
out_lines.append("Panel genes kept per cancer (after mean TPM>=0.5 filter):")
for cancer in TARGET_PROJECTS:
    out_lines.append(f"  {cancer}: " + "; ".join(
        f"{p}={','.join(gene_report[cancer][p]) if gene_report[cancer][p] else 'NONE'}"
        for p in PANELS))
out_lines.append("")
for tid, desc, eff, p in tests:
    out_lines.append(f"[{tid}] {desc}")
    if not (isinstance(p, float) and np.isnan(p)):
        out_lines.append(f"    effect = {eff:.4g}    P = {p:.3g}")
    else:
        out_lines.append(f"    effect = {eff:.4g}")
    out_lines.append("")
(RESULTS_DIR / "nc52_tcga_composition_excc.txt").write_text(
    "\n".join(out_lines), encoding="utf-8")

print("\n".join(out_lines))
print(f"\nSaved: {RESULTS_DIR/'nc52_tcga_composition_excc.csv'}")
print(f"Saved: {RESULTS_DIR/'nc52_tcga_composition_excc.txt'}")
