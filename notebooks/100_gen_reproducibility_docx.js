...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      p("Version: 0.4.1 (editable install from project root)"),
      ...
      new Table({
        ...
        rows: [
          ...
          tableRow(["k_n floor (minimum)", "0 (positivity guard only; min observed per-pair k_n: 1.1e-4 mouse, 6.3e-4 human, 7.8e-6 brain)", "single-cell: mouse, human, brain"], [3200, 1600, 4200]),
      ...
      p("where mu_ct = cell type\u2019s global mean omega, mu_pair = region pair\u2019s mean omega, mu_grand = global mean over all 31,764 pairs (32.56)."),
      ...
      p("     Calibrated omega: omega_cal = omega / 6.67 (empirical baseline from mouse split-half, n=6; 95% bootstrap CI [4.24, 9.24] computed by resampling the 6 control omega values with replacement, B = 10,000, reporting the 2.5th and 97.5th percentiles — see results/mouse_pilot_v2_results.csv, category C_control). Rescales all values so equivalent populations yield omega_cal ~ 1.0; given the width of the baseline CI, calibrated values are reported with one significant figure: brain global mean 32.56 -> omega_cal ~ 5 (range 3.5-7.7); astrocytes 76.83 -> ~ 12 (range 8.3-18.1); Bergmann glia 11.17 -> ~ 2. Scheme-matched split-half calibration inside the brain atlas gave an internal baseline of 12.29 (95% CI [12.12, 12.47]), ~1.8-fold higher than the mouse factor, under which Bergmann glia corresponds to omega_cal ~ 0.9 (at, not above, the internal baseline); the Tabula Sapiens internal baseline was 7.67 (95% CI [7.39, 8.00]), inside the mouse CI, so the mouse factor transfers to Tabula Sapiens but not to the brain dataset (omega_cal is dataset-relative). Function: cki.calibrate_omega(). Output: results/brain_bs_null_observed_pairs.csv (the authoritative observed-pair values; the earlier results/phaseC_calibration.json and results/phaseC_calibrated_omega_brain.csv predate the block-shuffle pipeline and are superseded)."),
      p(""),
      ...
      p("     Script: notebooks/09c_phaseC_methodological.py"),
      p("     Per-pair k_n across 31,764 brain comparisons: CV=92.89% (mean=0.0027, median=0.0017). Spearman rho between per-pair omega and global-kn omega = +0.181 (P=9.38e-232), confirming pair-specific k_n yields substantially different rankings. Justifies the per-pair k_n estimator used in all reported analyses. Output: results/phaseC_kn_variability.json, results/phaseC_kn_stats.csv, results/phaseC_omega_pair_vs_global.csv."),
      ...
      p(""),
      p("  k. omega 32.56 vs 76.83 (M-S6): Clarified in Results that grand mean is lower because it is dominated by cell types with many pairs and low omega."),
      p(""),
      ...
      p("     Re-derived per-pair k_f and k_n for all 31,764 brain pairs and compared four aggregation schemes. Class ordering is stable under an aggregate-first k_n estimator (Spearman rho = 0.988 vs. the reported per-pair estimator; gradient 6.97-fold vs. 6.88-fold) but not under a single global k_n (rho = -0.21; this variant is effectively a rescaling of k_f-only). The astrocyte-vs-Bergmann-glia contrast is predominantly a k_n effect (k_f differs 1.2-fold, k_n 5.7-fold). The same script performed scheme-matched split-half calibration inside the brain atlas: internal baseline 12.29 (95% bootstrap CI [12.12, 12.47]; 29 populations, B = 50 splits each, >= 200 nuclei), ~1.8-fold higher than the mouse-derived 6.67, so the mouse calibration overstates omega_cal in the brain dataset. Outputs: results/reviewer_kn_estimator_consistency.csv, results/reviewer_brain_splithalf_summary.txt, results/reviewer_brain_splithalf_raw.csv."),
      p(""),
      ...
      p("     Oligodendrocyte-lineage classes account for 40.2% of analyzed pairs but 50 of 55 Strong candidates (fold 2.26; hypergeometric P = 4.5e-15 over the 31,764 pairs; permutation P = 1.0e-5 with B = 100,000; OPCs alone P = 1.2e-7). Output: results/reviewer_lineage_enrichment.txt."),
      p(""),
      p("  e. Tier-threshold sensitivity (C-D):"),
      p("     Script: notebooks/39_reviewer_fix_tier_sensitivity.py"),
      p("     Recomputed the Strong set and lineage enrichment over a 19-combination grid of residual caps (0.2-0.4) and omega caps (12-25). 16 of 19 combinations show significant lineage enrichment; failures occur only at residual cap 0.2 (a single candidate), and at residual caps >= 0.25 the maximum hypergeometric P is 1.4e-3. Output: results/reviewer_tier_sensitivity.csv."),
      p("     Script: notebooks/44_fix_phaseB_cis.py"),
      p("     Recomputed results/phaseB_bootstrap_cis.csv from the authoritative omega sources: brain rows from results/brain_bs_null_observed_pairs.csv (astrocyte mean 76.83, matching the main text) and human rows from results/phase35_all_metrics_pairs.csv (n = 4,851); removed 16 legacy Human (per-CT) rows computed with a null-sd approximation; mouse rows preserved verbatim (downstream loaders depend on them). The data sources of notebooks/09_phaseB_statistical_upgrades.py were updated accordingly, and results/phaseC_calibrated_cis.csv was regenerated (deterministic division by 6.67)."),
      ...
      p("     Fixed gene-panel ablation of the brain landscape: recomputed all 31,764 same-cell-type cross-region pairs under four k_f gene-selection schemes (S0 reported per-pair top-200 DE; S1 fixed top-2,000 by global mean; S2 leave-pair-out top-200; S3 all 5,000 non-HK genes), with identical keep set, pseudobulks and k_n. The reference implementation reproduced the reported landscape exactly (max per-pair omega difference 2.8e-12). Pair-level Spearman rho vs S0: 0.958 (S1), 0.964 (S2), 0.959 (S3); class-mean ordering rho 0.95/0.99/0.93; astrocyte/Bergmann-glia ratio 6.88 (S0) versus 7.96/10.50/6.48. Circularity inflation: median k_f(S0)/k_f(S2) = 1.64 (IQR 1.27-2.16). Scheme-matched block-shuffle null under S2 (B = 200): 8 classes at the P = 0.00498 floor, choroid plexus P = 0.741, Bergmann glia P = 0.0995. Outputs: results/fixed_panel_ablation_pairs.csv, results/fixed_panel_ablation_ct.csv, results/fixed_panel_ablation_null_<CT>.npy, results/fixed_panel_ablation_summary.json."),
      ...
      code("      results/brain_siletti_migration_candidates_v3.csv # region-associated candidate list"),
      p(""),
      ...
      code("      results/brain_bs_null_summary.txt          # FDR summary (Strong n=55, 37 raw P<0.05, min q=0.949)"),
      ...
      p("[\u2713] Install CKI v0.4.1: pip install -e ."),
      ...
      p("[\u2713] Block-shuffle null: Verify B=1,000 in results/brain_bs_null_fdr_manifest.json (55 Strong candidates, 37 with raw P<0.05, minimum q=0.949). (Section 5.3)"),
      p("[\u2713] Phase B: Verify omega distribution characterization in results/phaseB_omega_distribution.json. (Section 5.3)"),
      p("[\u2713] Phase C: Verify calibrated omega (omega/6.67, 95% bootstrap CI [4.24, 9.24]): brain mean 32.56 -> omega_cal ~ 5, astrocytes 76.83 -> ~ 12, Bergmann glia 11.17 -> ~ 2 (results/brain_bs_null_observed_pairs.csv); verify internal split-half baselines 12.29 [12.12, 12.47] (brain) and 7.67 [7.39, 8.00] (Tabula Sapiens) in results/reviewer_brain_splithalf_summary.txt and results/reviewer_ts_splithalf_summary.txt. (Section 5.4/5.6)"),
      p("[\u2713] Phase C: Verify dimensionality invariance in results/phaseC_dimensionality.json (ratio ~1.001). (Section 5.3)"),
      p("[\u2713] Phase C: Verify k_n variability (CV=92.89%) in results/phaseC_kn_variability.json. (Section 5.4)"),
