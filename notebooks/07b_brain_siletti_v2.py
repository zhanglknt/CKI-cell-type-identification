"""
CKI Brain Analysis v2: Siletti et al. (2023) Non-neuronal nuclei
==============================================================
FIX: Skip detect_housekeeping_genes() on full 888K-cell matrix.
      Load HRT Atlas reference directly as HK genes (human data).
...
# ===== 2. HK genes: load HRT Atlas directly (skip detect on full matrix) =====
print("\n" + "=" * 60)
print("2. Loading HK genes from HRT Atlas reference...")
print("=" * 60)

# Load HRT Atlas human genes
hk_df = pd.read_csv(HK_FILE, sep=";", engine="python")
hk_genes = set(hk_df["Human"].dropna().astype(str))
print(f"  HRT Atlas human genes: {len(hk_genes)}")

# Map to adata gene names
gene_names = adata.var_names.tolist()
hk_indices = [i for i, g in enumerate(gene_names) if g in hk_genes]
print(f"  HK genes found in adata: {len(hk_indices)}")

if len(hk_indices) < 100:
    print("  WARNING: too few HK genes found! Trying combined detect on subsample...")
    # Subsample to 50,000 cells for HK detection
    import scanpy as sc_sub
    rng_sub = np.random.RandomState(RANDOM_SEED)
    sub_idx = rng_sub.choice(adata.n_obs, min(50000, adata.n_obs), replace=False)
    adata_sub = adata[sub_idx].copy()
    from cki.gene_sets import detect_housekeeping_genes
    hk_result = detect_housekeeping_genes(adata_sub, method="combined", use_reference=True, species="human")
    hk_genes_sub = hk_result["genes"]
    hk_indices = genes_to_indices(adata, hk_genes_sub)
    print(f"  HK genes from subsample detect: {len(hk_indices)}")

print(f"  Final HK indices: {len(hk_indices)}")
...
for _, row in groups_ok.iterrows():
    ...
    if hasattr(group_adata.X, "toarray") and group_adata.n_obs < 5000:
        pb = np.asarray(group_adata.X.mean(axis=0)).flatten()
    elif group_adata.n_obs < 5000:
        pb = np.asarray(group_adata.X.mean(axis=0)).flatten()
    else:
        # For large groups, compute mean on sparse matrix efficiently
        X = group_adata.X
        if hasattr(X, "toarray"):
            # Sparse - use matrix multiplication for efficiency
...
# Build full gene vector for mapping (need to know gene count)
# We need the gene count - get it from any pseudobulk
any_pb = next(iter(pseudobulk_data.values()))
N_GENES = len(any_pb)
print(f"  Gene vector length: {N_GENES}")
...
# ===== 7. Multiplicative migration detection =====
print("\n" + "=" * 60)
...
pairs_csv = RESULTS_DIR / "brain_siletti_omega_pairs_v2.csv"
pairs_df.to_csv(pairs_csv, index=False)
print(f"  Saved: {pairs_csv}")

migration_csv = RESULTS_DIR / "brain_siletti_migration_candidates_v2.csv"
migration_df.to_csv(migration_csv, index=False)
print(f"  Saved: {migration_csv}")

summary_csv = RESULTS_DIR / "brain_siletti_ct_summary_v2.csv"
...
report_lines = []
report_lines.append("# CKI Brain Analysis Report (v2 - HRT Atlas HK)\n\n")
...
report_md = RESULTS_DIR / "brain_siletti_analysis_report_v2.md"
...
kv_csv = RESULTS_DIR / "brain_siletti_key_values_v2.csv"
