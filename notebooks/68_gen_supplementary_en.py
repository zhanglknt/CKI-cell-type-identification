...
add_para('CKI: A Cell-type Identity Index for Quantifying Baseline-Normalized Transcriptomic Remodeling')
add_para('Li Zhang')
add_para('')
...
add_para(
    ...
    '1.1e-4, mouse; 6.3e-4, human; 7.8e-6, brain). '
...
add_para(
    ...
    'cluster-aware intervals for landscape-level quantities: gradient 6.88 [5.68, 9.15], '
    'grand mean \u03c9 32.56 [30.48, 34.71], Strong-candidate count 55 [24, 93]. '
...
add_para(
    ...
    f'(residual < 0.3), {_br.get("n_significant", 37)} showed raw P < 0.05, but after Benjamini-Hochberg '
    f'correction across all m = 31,764 pairs, none reached q < 0.05 (minimum q = {_br["min_q_fdr"]:.3f}). '
    f'Globally, {_bs["n_p_lt_05"]:,} of 31,764 pairs showed raw lower-tail P < 0.05, fewer than the ~1,588 (5%) '
    f'expected under a global null. The upper tail shows the complementary excess: '
    f'{_bs.get("n_p_high_lt_05", 6153):,} pairs ({_bs.get("pct_p_high_lt_05", 19.4):.1f}%) had upper-tail '
    f'P < 0.05 (observed \u03c9 above the block-shuffle null), concentrated in the most regionally structured '
    f'classes (astrocytes 23.3%, oligodendrocytes 22.0%, microglia 20.4% of class pairs). The per-pair '
    f'P-value distribution is therefore shifted upward\u2014a lower-tail deficit alongside an upper-tail '
    f'excess\u2014which is the pair-level signature of the same regional structure detected at the '
    f'cell-class level and is not evidence for anomalously low-\u03c9 pairs in aggregate. '
    ...
    'selection-rule empirical FDR of 0.66\u20130.72 quantifies. '
...
add_para(
    ...
    f'\u2248 {_br["global_mean"] / _mc["control_mean"]:.0f} (raw {_br["global_mean"]:.2f}; range 3.5\u20137.7 across the baseline CI), and the most divergent brain cell type '
    ...
    f'{_brain_sh_mean:.2f} (95% bootstrap CI [{_brain_sh_ci[0]:.2f}, {_brain_sh_ci[1]:.2f}]), approximately 1.8-fold '
...
add_para(
    ...
    'split-half calibration baseline (12.29), supporting external validity.'
)
add_para(
...
add_para(
    ...
    'per-pair omega difference 2.8e-12 over 31,764 pairs). A scheme-matched '
    'block-shuffle null (B = 200) was rerun under S2. Script: '
    'notebooks/46_fixed_panel_ablation.py (runtime about 28 minutes).'
)
add_para(
    'Rank robustness. Pair-level Spearman correlation with S0: rho = 0.958 (S1), '
    '0.964 (S2), 0.959 (S3). Ordering of the ten class means: rho = 0.95 (S1), 0.99 '
    '(S2), 0.93 (S3); under S2 the only change was a swap of the adjacent ependymal '
    'and fibroblast means. Astrocyte-to-Bergmann-glia ratio: 6.88 (S0) versus 7.96 '
    '(S1), 10.50 (S2), 6.48 (S3) - the gradient is preserved and amplified under the '
    ...
    'block-shuffle null, the same eight classes reached the permutation floor '
    '(P = 0.00498); choroid plexus remained non-significant (P = 0.741, versus 0.761 '
    'reported); Bergmann glia, marginal in the reported scheme (P = 0.031), was not '
    'significant under S2 (P = 0.0995).'
)
add_para(
    'Circularity inflation and scale. On the same pairs, the circular panel '
    'inflated k_f by a median of 1.64-fold relative to the leave-pair-out panel '
    '(IQR 1.27-2.16) and by 7.0- to 7.9-fold relative to the fixed and unselected '
    'panels (which also differ in panel composition). The absolute omega scale is '
    'therefore scheme-specific (grand mean 32.56 for S0 versus 24.75, 4.89, and 4.08 '
    'for S1-S3). Rank-based conclusions are robust: the multiplicative-residual '
    'ranking correlated at rho = 0.94-0.95 across schemes, and S2 retained 173 of '
    '176 (98.3%) of the S0 residual < 0.3 candidate pairs. Absolute tier cutoffs '
...
s4_text = (
    ...
    f'{pct_weak:.2f}%). '
    f'Under the block-shuffle permutation null (B = {_bs["B"]:,}), {_br.get("n_significant", 37)} of the {n_strong} '
