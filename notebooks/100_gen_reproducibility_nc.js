 ...
 * Nature Communications-compliant DOCX with Arial font, same formatting as manuscript.
 */
const fs = require("fs");
const {
...
const OUT = "C:/Users/KnightZ/Desktop/细胞受选择/results/CKI_Reproducibility_Guide_NC.docx";
const FONT = "Arial";
const BLACK = "000000";
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      code("seaborn:             0.13.2"),
      code("statsmodels:         0.14.6"),
      code("meld:                1.0.2   (perturbation benchmark, notebooks/101)"),
      code("pyaugur:             0.1.0   (pure-Python Augur port, notebooks/91)"),
      ...
      p("Version: 0.5.0 (editable install from project root)"),
      ...
      heading("4.3 Real-Data Drift Calibration (Kang IFN-\u03b2 PBMC + Siletti Brain Ladder) \u2014 Result 4 (Fig. 3)", 3),
      p("Datasets:    Kang et al. (Nat. Biotechnol. 2018) IFN-\u03b2 PBMC cohort, batch 1 (same-donor, same-condition cross-lane technical replicates); Siletti et al. brain atlas (same processing as Section 4.5)."),
      p("Kang cross-lane technical replicates (nc49_pilot_kang_techrep.py):", { bold: true }),
      p("  1. Batch-1 cells grouped by (donor, condition, cell type); groups with at least 50 cells per lane retained, yielding 30 cross-lane pairs."),
      p("  2. Per-pair pseudobulks, k_n on the shared HK gene set, k_f on per-pair top-200 DE genes (hybrid mode, Section 3.2)."),
      p("  3. n-matched cell-shuffle null (B = 200 per pair); calibration = observed / own null median; FPR = observed > own null 95th percentile."),
      p("  4. Same statistic computed for k_n, k_f, omega, raw JS, cosine. Output: results/nc49_pilot_kang_techrep.csv (omega 0/30 misreports; raw JS 36.7%, cosine 23.3%)."),
      p("Brain drift ladder (nc49_brain_drift_ladder.py):", { bold: true }),
      p("  1. Three pair tiers of escalating drift: T1 same (donor, region) cross-library pairs (n = 2,161, pure technical drift), T2 same-region cross-donor pairs (n = 1,089), T3 same-donor cross-region pairs (n = 1,656, positive control)."),
      p("  2. Per-pair n-matched cell-shuffle null (B = 100 for T1, 30 for T2 and T3, matching the manuscript Methods); calibration ratio and FPR computed for seven metrics: k_n, k_f, omega, raw JS, cosine, Spearman, marker Jaccard."),
      p("  3. omega shows the lowest T1 misreporting among the continuous divergence metrics (28.6% versus 35.7\u201345.2%; below raw JS in 10/10 cell classes); marker Jaccard is lower still on the FPR statistic (19.9%) but responds weakest to genuine regional divergence (T3 calibration 1.41 versus 1.80 for omega)."),
      p("  Output: results/nc49_brain_drift_ladder.csv. Figures: notebooks/nc49_fig_drift_ladder.py (Fig. 3; results/figures_final/nc49_fig_drift_ladder.pdf/.png)."),
      p(""),
      ...
      p("  7. NN/TT ratio computed as mean(omega_NN) / mean(omega_TT) per cancer type under the linear probability mapping (the manuscript and Fig. 4a report this mean NN/TT omega ratio with sample-level cluster-bootstrap 95% CIs; the legacy median/softmax caliber is archived as a sensitivity analysis in Section 1.7 of the Supplementary Information)."),
      ...
      p("Per-sample statistics (nc49_tcga_main.py, seed 42):", { bold: true }),
      p("  Per-tumor statistics are the mean of omega, k_f, k_n over all linear-normalization pairs a sample participates in (35,306 pairs). Group ratios carry sample-level cluster-bootstrap 95% CIs (B = 1,000; tumor/normal resampled independently, pairs reweighted by endpoint-weight products). LUAD driver groups tested with Kruskal-Wallis + Dunn post-hoc (Holm); group mean differences carry within-group bootstrap CIs. Pair-level Mann-Whitney P-values are descriptive only (dyadic dependence). Outputs: results/nc49_tcga_pancancer.csv, results/nc49_tcga_luad_mutation.csv. LIHC Cox models: notebooks/nc49_pilot_lihc_cox.py (results/nc49_pilot_lihc_cox.csv). Figure: notebooks/nc49_fig_tcga.py (Fig. 4; results/figures_final/nc49_fig_tcga.pdf/.png)."),

      p("Cross-organ conservation (Result 6, Fig. 5): Subset of 59 same-cell-type cross-organ pairs from Tabula Sapiens data. Ranked by mean omega per cell type."),

      heading("4.5 Siletti Brain Atlas (Human) \u2014 Result 7 (Fig. 6)", 3),
      ...
      p(""),
      p("  j. Parameter justification (M-M5): Added as Limitation #15 and Section 3.9 of the Supplementary Information (Parameter Justification) (normalization pseudo-count, epsilon, top-200 DE, HVG count, log-base 2, B=1000)."),
      p(""),
      ...
      // 5.10 V49 ANALYSES
      // ========================================================
      heading("5.10 v49 Analyses", 3),
      p("The following analyses were added for the v49 revision (real-data drift calibration and TCGA per-sample statistics). All reuse the authoritative gene sets, pseudobulks, and null frameworks of the corresponding main analyses; no result above is altered."),
      p(""),
      p("  a. Kang cross-lane technical-replicate calibration:"),
      p("     Script: notebooks/nc49_pilot_kang_techrep.py"),
      p("     30 same-donor, same-condition cross-lane pairs from Kang batch 1 (at least 50 cells per group); per-pair n-matched cell-shuffle nulls (B = 200); calibration = observed / own null median, FPR = observed > own null 95th percentile. omega misreported 0 of 30 pairs (raw JS 36.7%, cosine 23.3%; k_f alone also 0 of 30). Output: results/nc49_pilot_kang_techrep.csv."),
      p(""),
      p("  b. Brain drift ladder:"),
      p("     Script: notebooks/nc49_brain_drift_ladder.py"),
      p("     Three tiers of escalating drift (T1 same-donor same-region cross-library, 2,161 pairs; T2 cross-donor, 1,089; T3 cross-region, 1,656), seven metrics per pair with n-matched nulls (B = 100 for T1, 30 for T2 and T3). omega T1 FPR 28.6%, lowest among the continuous divergence metrics (raw JS 45.2%, cosine 44.1%, Spearman 40.6%, k_f 37.6%, k_n 35.7%); marker Jaccard is lower still (19.9%) but responds weakest to genuine regional divergence (T3 calibration 1.41 versus 1.80 for omega and 2.98 for raw JS). Output: results/nc49_brain_drift_ladder.csv."),
      p(""),
      p("  c. TCGA per-sample statistics:"),
      p("     Scripts: notebooks/nc49_tcga_main.py (seed 42), notebooks/nc49_pilot_lihc_cox.py, notebooks/nc49_tcga_purity.py, notebooks/nc49_tcga_luad_smoking.py"),
      p("     Per-sample means of omega/k_f/k_n over the 35,306-pair linear-normalization table; pan-cancer NN/TT mean ratios with sample-level cluster-bootstrap 95% CIs (B = 1,000); LUAD driver-group Kruskal-Wallis + Dunn-Holm contrasts with bootstrap difference CIs; LIHC overall-survival Cox models (six exposures). Covariate sensitivity: ESTIMATE stromal+immune (141+141 genes) admixture scoring (per-cancer k_n/omega ~ admixture regressions, LUAD ANCOVA, high-purity-half NN/TT bootstrap) and LUAD smoking adjustment (cBioPortal study luad_tcga patient-level data; 427/492 tumors with known status; OLS with ever-smoker, age/sex, and admixture covariates). k_f panel semantics: Hallmark enrichment of the LUAD KRAS TT identity panels (no program at FDR < 0.05) and pair-level log k_f composition regressions (TT >= NN k_f retained in 5/5 cancers). Outputs: results/nc49_tcga_pancancer.csv, results/nc49_tcga_luad_mutation.csv, results/nc49_pilot_lihc_cox.csv, results/nc49_tcga_purity.csv, results/nc49_tcga_admix_scores.csv, results/nc49_tcga_luad_smoking.csv, results/nc49_tcga_kf_composition.csv (notebooks/nc49_tcga_kf_composition.py)."),
      p(""),
      p("  d. v49 main figures:"),
      p("     Scripts: notebooks/nc49_fig_drift_ladder.py (Fig. 3), notebooks/nc49_fig_tcga.py (Fig. 4)"),
      p("     Outputs: results/figures_final/nc49_fig_drift_ladder.pdf/.png, results/figures_final/nc49_fig_tcga.pdf/.png."),
      p(""),
      ...
      p(""),
      p("    v49 analyses (Section 5.10):"),
      code("      results/nc49_pilot_kang_techrep.csv              # Kang cross-lane technical-replicate calibration (Section 5.10a)"),
      code("      results/nc49_brain_drift_ladder.csv              # brain drift ladder, 7 metrics x 3 tiers (Section 5.10b)"),
      code("      results/nc49_tcga_pancancer.csv                  # pan-cancer NN/TT mean ratios + cluster-bootstrap CIs (Section 5.10c)"),
      code("      results/nc49_tcga_luad_mutation.csv              # LUAD driver-group contrasts (Section 5.10c)"),
      code("      results/nc49_pilot_lihc_cox.csv                  # LIHC Cox sensitivity models (Section 5.10c)"),
