...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      p("Version: 0.5.4 (editable install from project root)"),
      ...
      new Table({
        ...
        rows: [
          ...
          tableRow(["Calibrated omega baseline", "7.70, two-stage bootstrap 95% CI [6.38, 9.82] (mouse split-half, 50 replicates across six control populations, 300 omega values; B = 5,000 two-stage interval supersedes the pseudo-replicated [7.37, 8.02]; data: results/mouse_splithalf_v44.csv, results/nc52_stats_baseline_twostage_bootstrap.csv)", "Phase C (C-M1)"], [3200, 1600, 4200]),
      ...
      p("  LIHC Edmondson grade (ex-CC, v52): G1 (n=39), G2 (n=134), G3 (n=105), G4 (n=11). Jonckheere-Terpstra trend test."),
      ...
      p("Per-sample statistics (nc52_tcga_excc_main.py, seed 42; ex-CC default since v52):", { bold: true }),
      p("  Per-tumor statistics are the mean of omega, k_f, k_n over all linear-normalization pairs a sample participates in (34,828 ex-CC pairs: the 478 pairs touching the 32 cell-line-derived aliquots dropped from the 35,306-pair table). Group ratios carry sample-level cluster-bootstrap 95% CIs (B = 1,000; tumor/normal resampled independently, pairs reweighted by endpoint-weight products). LUAD driver groups tested with Kruskal-Wallis + Dunn post-hoc (Holm); group mean differences carry within-group bootstrap CIs. Pair-level Mann-Whitney P-values are descriptive only (dyadic dependence). Outputs: results/nc52_tcga_pancancer_excc.csv, results/nc52_tcga_excc_severity.csv, results/nc49_tcga_luad_mutation.csv. LIHC Cox models: notebooks/nc52_lihc_cox_excc.py (R survival::coxph, AJCC stage categorical, cox.zph PH check; results/nc52_lihc_cox_excc.csv, results/nc52_lihc_cox_excc_zph.csv). Figure: notebooks/nc49_fig_tcga.py (Fig. 4; results/figures_final/nc49_fig_tcga.pdf/.png)."),
      ...
      p("     Calibrated omega: omega_cal = omega / 7.70 (empirical baseline from 50 split-half replicates across the six mouse control populations, 300 omega values; two-stage bootstrap 95% CI [6.38, 9.82], B = 5,000, scripts/nc52_stats_resampling.py, superseding the pseudo-replicated [7.37, 8.02]; data: results/mouse_splithalf_v44.csv). Rescales all values so equivalent populations yield omega_cal ~ 1.0; given the width of the two-stage baseline CI, calibrated values carry at most about one significant figure (resolution below ~0.1-0.2 in omega_cal is not meaningful): brain global mean 38.55 -> omega_cal ~ 5; astrocytes 82.75 -> ~ 11; Bergmann glia 13.56 -> ~ 1.8. Scheme-matched split-half calibration inside the brain atlas gave an internal baseline of 9.73 (95% CI [9.03, 10.53]), ~1.3-fold higher than the mouse factor, under which Bergmann glia corresponds to omega_cal ~ 1.4 (close to the internal baseline); the Tabula Sapiens internal baseline was 7.67 (95% CI [7.39, 8.00]), inside the two-stage mouse CI [6.38, 9.82], so the mouse factor transfers to Tabula Sapiens but not to the brain dataset (omega_cal is dataset-relative). The legacy six-split estimate 6.67 (95% bootstrap CI [4.24, 9.24], from results/mouse_pilot_v2_results.csv, category C_control) is consistent with the updated 50-replicate estimate (CIs overlap) and is superseded. Function: cki.calibrate_omega(). Output: results/brain_bs_null_observed_pairs.csv (the authoritative observed-pair values; the earlier results/superseded/phaseC_calibration.json and results/superseded/phaseC_calibrated_omega_brain.csv predate the block-shuffle pipeline and are superseded)."),
      p(""),
      ...
      p("     Repeats the split-half calibration 50 times across the six FACS control populations (300 omega values), yielding the updated calibration constant 7.70 (two-stage bootstrap 95% CI [6.38, 9.82], B = 5,000, scripts/nc52_stats_resampling.py; supersedes the pseudo-replicated [7.37, 8.02] and the legacy six-split estimate 6.67). Outputs: results/mouse_splithalf_v44.csv, results/mouse_splithalf_v44_summary.json, results/nc52_stats_baseline_twostage_bootstrap.csv."),
      p(""),
      ...
      p("     Superseded in v52 by the ex-CC default chain (Section 5.13a-e); retained as the historical v49 record. Scripts: notebooks/nc49_tcga_main.py (seed 42), notebooks/nc49_pilot_lihc_cox.py, notebooks/nc49_tcga_purity.py, notebooks/nc49_tcga_luad_smoking.py"),
      ...
      p("     Astrocyte comparisons restricted to the intra-cerebellar subset (21 region pairs, the identical 7-region set available to Bergmann glia) so that the astrocyte-Bergmann glia contrast is matched on anatomical span rather than only on class size. Per-region-pair ratios give a span-matched gradient of 3.68 (paired median 4.30, 95% CI [3.40, 4.95]; bootstrap B = 10,000) versus 6.10 on the full data - the gradient survives span matching with reduced magnitude. Includes the k_f/k_n decomposition and vascular-cell reference on the same subset. Outputs: results/nc49_brain_region_matched.csv, results/nc49_brain_region_matched_summary.csv, results/nc49_brain_region_matched.txt."),
      p(""),
      p("  b. Equal-n gradient k_f/k_n decomposition:"),
      p("     Script: notebooks/96_brain_downsample_decomp_v49.py"),
      p("     Line-by-line mirror of the v44 equal-n downsampling pipeline (4,118 nuclei per class, 20 replicates, seed 42 - identical draw sequence) that additionally records per-class k_f and k_n in every replicate. Reproduces the published omega gradient (1.7406, 95% CI [1.635, 1.836]); the astrocyte/Bergmann-glia k_f ratio under equal-n is 2.09 (95% CI [2.02, 2.18]) - essentially the full-data value (2.03) - whereas the k_n ratio reverses direction (equal-n 1.29, 95% CI [1.21, 1.41], astrocyte higher; full-data 0.31, Bergmann glia higher): the size-balanced residual gradient is carried by k_f, and the Bergmann-glia k_n elevation in the full data is largely a class-size artefact. Outputs: results/brain_v49_downsample_replicates_kfkn.csv, results/brain_v49_downsample_kfkn_summary.json."),
      p(""),
      p("  c. Calibration leave-one-out sensitivity:"),
      p("     Script: notebooks/92_calib_leave_one_out_v49.py"),
      p("     Leave-one-population-out refits of the split-half calibration baseline (7.70): excluding hepatocytes (12.44 +/- 5.98 versus 5.77-7.36 for the other five populations) gives 6.75 (-12.3%); LOO range [6.75, 8.08], median baseline 7.12. Omega_cal anchor recalibration moves absolute values by at most ~14% and no ratio-based conclusion changes. Outputs: results/nc49_calib_leave_one_out.csv, results/nc49_calib_leave_one_out.txt."),
      p(""),
      p("  d. LUAD mutation whole-tumor label permutation:"),
      p("     Script: notebooks/93_luad_group_permutation_v49.py"),
      p("     The per-tumor LUAD driver-group statistics (Kruskal-Wallis and pairwise contrasts, specification identical to notebooks/nc49_tcga_main.py) recomputed with whole-tumor label permutation (B = 10,000, seed 42) in place of asymptotic/parametric P-values, breaking any within-tumor pair dependence assumption. KW P = 0.0001; pairwise KRAS-WT P = 0.0001, KRAS-EGFR P = 0.0034, EGFR-WT P = 0.21 - the published ordering and significance pattern is preserved. Outputs: results/nc49_tcga_luad_mutation_perm.csv."),
      p(""),
      ...
      p("     Barcode source-code audit of all TCGA samples: every sample in the five cancers carries source codes 01/11 (primary tumour/normal) except 32 LIHC samples barcoded with the cell-line-derived CC code - the sole anomalous source across the panel. These 32 CC samples are the previously documented contaminant set; excluding them from the high-purity-half analysis moves the LIHC NN/TT omega ratio from 1.167 to 1.188 (95% CI [1.005, 1.435]) - direction and conclusion unchanged. The audit ledger also records CC-touching composition pairs (291 of 2,000 LIHC TT pairs) and the 32 CC patients in the clinical JSON (377 total). In v52 this exclusion became the default cohort for every TCGA analysis (Section 5.13). Outputs: results/nc49_cc_barcode_audit.csv, results/nc49_cc_excl_sensitivity.csv, results/nc49_cc_audit_report.txt."),
      p(""),
      ...
      p("     The published LIHC Cox family (omega/k_f/k_n/TN-omega exposures; M1-M6) refit with all 32 ILSBio CC tumours excluded (n = 272 versus 304): the null is unchanged (omega HR/SD 1.083 [0.870, 1.349], P = 0.47, versus 1.075 [0.882, 1.309], P = 0.48 with all tumours). Superseded in v52: ex-CC is now the default cohort and the Cox family is refit with R survival::coxph (Section 5.13b). Outputs: results/nc49_lihc_cox_excc.csv, results/audit/nc49_lihc_cox_excc_2026-09-21.md."),
      p(""),
      ...
      // ========================================================
      // 5.13 NC52 ANALYSES (v52 REVISION: ex-CC DEFAULT + PANEL RECOMPUTES)
      // ========================================================
      heading("5.13 nc52 Analyses", 3),
      p("The following analyses were added for the v52 revision (expert-panel round on v51r2). The ex-CC cohort (all 32 ILSBio cell-line-derived aliquots excluded by the barcode audit of Section 5.11f) is now the DEFAULT for every TCGA analysis; pre-exclusion outputs are archived under results/superseded/. All scripts use seed 42 and write results/ files with the nc52_ prefix."),
      p(""),
      p("  a. TCGA ex-CC main chain:"),
      p("     Script: notebooks/nc52_tcga_excc_main.py"),
      p("     Pan-cancer NN/TT omega ratios (sample-level cluster-bootstrap 95% CIs, B = 1,000), clinical-severity stratifications (LIHC Edmondson JT, BRCA PAM50 KW, LUAD mutation KW), and the summary ledger on the ex-CC pair table (34,828 pairs; 478 CC-touching pairs dropped from 35,306). LUAD 2.46 > KIRC 1.88 > LUSC 1.71 > BRCA 1.57 > LIHC 1.11 [0.94, 1.30] (CI includes 1; the four-of-five statement is preserved). Outputs: results/nc52_tcga_pancancer_excc.csv, results/nc52_tcga_excc_severity.csv, results/nc52_tcga_excc_composition.csv, results/nc52_tcga_excc_summary.json."),
      p(""),
      p("  b. LIHC ex-CC Cox family (R survival::coxph):"),
      p("     Script: notebooks/nc52_lihc_cox_excc.py"),
      p("     The LIHC Cox family (omega/k_f/k_n/TN-omega exposures) refit on the ex-CC default cohort (n = 272 tumors with complete covariates, 79 events) with R survival::coxph, AJCC stage as a categorical factor, Edmondson grade ordinal, age and sex; proportional hazards checked with cox.zph (M2 GLOBAL P = 0.023, reported). The omega null is unchanged (full-model HR/SD 1.08 [0.88, 1.33], P = 0.467). Outputs: results/nc52_lihc_cox_excc.csv, results/nc52_lihc_cox_excc_cohort.csv, results/nc52_lihc_cox_excc_zph.csv."),
      p(""),
      p("  c. Composition check, ex-CC default:"),
      p("     Script: notebooks/nc52_tcga_composition_excc.py"),
      p("     Four-panel marker composition check on 25,015 ex-CC pairs (291 CC-touching pairs dropped; CC samples also excluded from the z-score standardization population; B = 1,000 cluster bootstrap). |Delta z| TT/NN 1.305-fold (overall3, P = 2.63e-135) and 1.216-fold (overall4, P = 4.67e-68); myeloid panel alone null (1.001, P = 0.982); pooled attenuation -0.9% (bootstrap median -0.8% [-4.3%, +2.5%]); per-cancer LIHC +44.1%, KIRC +19.7%, BRCA -15.9%, LUSC -9.0%, LUAD -2.0%; Spearman rho(k_n, composition) 0.380 pooled (per-cancer 0.223-0.513). Outputs: results/nc52_tcga_composition_excc.csv, results/nc52_tcga_composition_excc.txt."),
      p(""),
      p("  d. LUAD adjusted-model whole-tumor permutation:"),
      p("     Script: notebooks/nc52_tcga_luad_adjmodel_permutation.py"),
      p("     The adjusted LUAD driver contrasts (OLS with ever-smoker, age/sex, and admixture covariates) re-run as whole-tumor label-permutation tests that re-fit the full model at every permutation (B = 10,000): KRAS contrasts P <= 0.001 in every model, EGFR-WT P >= 0.28, matching the parametric conclusions. Output: results/nc52_tcga_luad_adjmodel_permutation.csv."),
      p(""),
      p("  e. Mapping-scheme and kn_floor sensitivity:"),
      p("     Scripts: notebooks/nc52_tcga_softmax_pairs.py, notebooks/nc52_tcga_mapping_schemes.py, notebooks/nc52_tcga_knfloor_sensitivity.py"),
      p("     Two-caliber NN/TT table (Supplementary Table 5): linear four-of-five CIs exclude 1 (LIHC 1.11 [0.94, 1.30]) versus softmax five-of-five (LIHC 1.29 [1.09, 1.53]); kn_floor in {0, 1e-5, 1e-4} leaves every ratio identical (no pair-level k_n in (0, 1e-4)), while 1e-3 truncates ~50% of NN pairs and attenuates the reversal. Outputs: results/nc52_tcga_softmax_all_pairs.csv, results/nc52_tcga_mapping_schemes_table.csv, results/nc52_tcga_mapping_schemes_summary.json, results/nc52_tcga_knfloor_sensitivity.csv."),
      p(""),
      p("  f. Deconvolution feasibility (negative control documentation):"),
      p("     Script: notebooks/nc52_tcga_deconv_feasibility.py"),
      p("     Documents why reference-based deconvolution (BayesPrism/CIBERSORTx) is not executable in the locked environment (R 4.1.3 base packages only, no compiled extensions) and archives the per-tumor marker-based composition estimates used in its place. Outputs: results/nc52_tcga_deconv_feasibility.csv/.json, results/nc52_tcga_deconv_lihc_pertumor.csv, results/nc52_tcga_deconv_kirc_pertumor.csv."),
      p(""),
      p("  g. GTEx healthy reference:"),
      p("     Script: notebooks/nc52_gtex_kn.py"),
      p("     GTEx V8 gene-level TPM (lung n = 288, liver n = 110, kidney cortex n = 28, breast n = 179 after QC and seed-42 subsampling) processed with the identical k_n pipeline. Adjacent-normal k_n is at healthy-tissue levels (healthy/adjacent ratio ~1.0-1.2) in lung, liver, and breast while tumor k_n is 2.0-2.7-fold higher - arguing against a field-effect reading; kidney is an exception (n = 28, high variance). Cross-cohort GTEx-adjacent pairs carry a cohort technical effect (k_n at tumor levels, P <= 1.4e-26), so only within-cohort orderings are interpreted. Outputs: results/nc52_gtex_pairs.csv, results/nc52_gtex_kn_by_grouptype.csv, results/nc52_gtex_summary.json."),
      p(""),
      p("  h. Brain donor-level bootstrap and combined control:"),
      p("     Scripts: notebooks/nc52_brain_extract.py, notebooks/nc52_brain_donor_bootstrap.py"),
      p("     Donor-cluster bootstrap 95% CIs (B = 5,000; four donors) for the astrocyte/Bergmann-glia gradient: equal-n 1.74 [0.80, 2.34] (12.6% of resamples < 1; downgraded to a sensitivity analysis), span-matched 3.68 [1.67, 3.97], and the combined span- and size-matched control 3.66 (donor-level [1.92, 3.78]; leave-one-donor-out range 2.31-3.68, all > 1) - the combined control is the primary caliber reported in the manuscript. Outputs: results/nc52_brain_ab_cells.npz, results/nc52_brain_ab_cellmeta.csv, results/nc52_brain_gradient_donor_bootstrap.csv/.json, results/nc52_brain_combined_equaln_spanmatch.csv."),
      p(""),
      p("  i. Brain RNA-quality proxy regressions:"),
      p("     Scripts: notebooks/nc52_brain_quality_regression.py, notebooks/nc52_brain_quality_mito.py"),
      p("     Library-level quality proxies (sequencing depth, UMI counts, detected genes, mitochondrial fraction) regressed against the drift-ladder statistics: the class-level omega calibration is essentially unchanged (0.300 -> 0.308), the T3 coefficient attenuates (0.257 -> 0.202), and the astrocyte/Bergmann-glia gradient is 6.33-6.45 after proxy adjustment (not reduced) - the proxies that can be measured do not drive the gradient, while PMI/RIN-related degradation deformation cannot be excluded (Limitations). Outputs: results/nc52_brain_quality_classlib.csv, results/nc52_brain_quality_classregion.csv, results/nc52_brain_quality_regression_classlib.csv, results/nc52_brain_quality_regression_mito.csv, results/nc52_brain_quality_mito_classlib.csv, results/nc52_brain_quality_mito_classregion.csv."),
      p(""),
      p("  j. Brain candidate effect sizes under k_n estimators:"),
      p("     Script: notebooks/nc52_brain_candidate_effectsize.py"),
      p("     The 39 Strong candidates re-evaluated under alternative k_n estimators: only 3 retain the Strong tier under the global-k_n estimator - the candidate list is estimator-sensitive and is reported as hypothesis-generating. Output: results/nc52_brain_candidate_effectsize.csv."),
      p(""),
      p("  k. Resampling re-estimates (statistics):"),
      p("     Script: scripts/nc52_stats_resampling.py"),
      p("     (i) Two-stage bootstrap of the 7.70 calibration baseline (B = 5,000; populations resampled first, split-half replicates second): 95% CI [6.38, 9.82], superseding the pseudo-replicated [7.37, 8.02]; omega_cal carries ~1 significant figure. (ii) Tabula Sapiens entry-clustered bootstrap (99 entries resampled, B = 5,000): all four inter-metric correlation CIs still exclude 0; the P < 1e-145 caliber is retired. (iii) AUC 0.80 with DeLong 95% CI [0.770, 0.838] (matching the seed-cluster interval [0.771, 0.836]). (iv) omega-versus-k_f real-data increment: k_f false-positive rate 0.38 versus omega 0.00 under 4:1 cell-count imbalance, and 0.98 versus 0.55 in the Kang CD14+ monocytes; the k_n permutation floor is 0.524 versus the observed 0.089, falsifying the mathematical-necessity claim. Outputs: results/nc52_stats_baseline_twostage_bootstrap.csv/.json, results/nc52_stats_tabula_entrycluster.csv, results/nc52_stats_auc_ci_methods.csv, results/nc52_stats_omega_vs_kf_realdata.csv, results/nc52_stats_omega_kf_math_floor.csv."),
      p(""),
      ...
      code("      results/superseded/tcga_composition_v44.txt / .csv  # v49.14 composition check, superseded by the ex-CC recompute (Sections 5.8b, 5.13c)"),
      ...
      code("      results/mouse_splithalf_v44.csv / _summary.json  # 50-replicate split-half calibration, omega = 7.70, two-stage 95% CI [6.38, 9.82] (Section 5.8e)"),
      code("      results/nc52_stats_baseline_twostage_bootstrap.csv # two-stage bootstrap CI of the 7.70 baseline (Section 5.13k)"),
      ...
      code("      results/nc52_tcga_pancancer_excc.csv             # pan-cancer NN/TT mean ratios + cluster-bootstrap CIs, ex-CC default (Section 5.13a)"),
      code("      results/nc52_tcga_excc_severity.csv              # clinical-severity stratifications, ex-CC default (Section 5.13a)"),
      code("      results/nc49_tcga_luad_mutation.csv              # LUAD driver-group contrasts (Section 5.10c)"),
      code("      results/nc52_lihc_cox_excc.csv                   # LIHC Cox models, ex-CC default, R survival::coxph (Section 5.13b)"),
      ...
      code("      results/nc52_tcga_composition_excc.csv/.txt      # four-panel marker composition check, ex-CC default (Section 5.13c)"),
      code("      results/nc52_tcga_luad_adjmodel_permutation.csv  # adjusted-model whole-tumor permutation, B = 10,000 (Section 5.13d)"),
      code("      results/nc52_tcga_mapping_schemes_table.csv      # linear vs softmax NN/TT two-caliber table (Section 5.13e)"),
      code("      results/nc52_tcga_knfloor_sensitivity.csv        # kn_floor in {0, 1e-5, 1e-4} leaves all ratios identical (Section 5.13e)"),
      code("      results/nc52_gtex_kn_by_grouptype.csv            # GTEx healthy-reference k_n by pair type (Section 5.13g)"),
      code("      results/nc52_brain_gradient_donor_bootstrap.csv  # donor-cluster bootstrap CIs of the brain gradients (Section 5.13h)"),
      code("      results/nc52_brain_combined_equaln_spanmatch.csv # combined span- and size-matched control, primary caliber (Section 5.13h)"),
      p(""),
      ...
      p("[\u2713] Verify TCGA sample count n = 3,535 (ex-CC default) and NN/TT mean ratios 1.11\u20132.46 with cluster-bootstrap CIs excluding 1 in 4 of 5 cancers (results/nc52_tcga_pancancer_excc.csv). (Section 5.13)"),
