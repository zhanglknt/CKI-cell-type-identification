...
# ---------------------------------------------------------------------------
# Color palette (NAR-friendly, colorblind-safe)
# ---------------------------------------------------------------------------
C_BG = '#FFFFFF'
C_TEXT = '#2C2C2A'
C_SUBTEXT = '#5F5E5A'
...
def main():
    ...
    # Title
    # -----------------------------------------------------------------------
    ax.text(3.25, 8.90, 'CKI: A Cell-state Kinetic Index',
    ...
    # -----------------------------------------------------------------------
    y_pb = 6.50
    draw_arrow(ax, 1.8, y_cells - 0.45, 2.3, y_pb + 0.25, color=C_SUBTEXT, lw=1)
    draw_arrow(ax, 4.7, y_cells - 0.45, 4.2, y_pb + 0.25, color=C_SUBTEXT, lw=1)

    box_pb = FancyBboxPatch((1.85, y_pb - 0.25), 2.8, 0.50, boxstyle='round,pad=0.05,rounding_size=0.1',
                            facecolor=C_PANEL, edgecolor=C_GRAY, linewidth=1)
    ax.add_patch(box_pb)
    # mini bar chart
    ...
    box_hk = FancyBboxPatch((0.55, y_split - 0.55), 2.4, 1.05, boxstyle='round,pad=0.05,rounding_size=0.12',
                            facecolor=C_BLUE_L, edgecolor=C_BLUE_M, linewidth=1)
    ax.add_patch(box_hk)
    ...
    box_fn = FancyBboxPatch((3.55, y_split - 0.55), 2.4, 1.05, boxstyle='round,pad=0.05,rounding_size=0.12',
                            facecolor=C_ORANGE_L, edgecolor=C_ORANGE_M, linewidth=1)
    ax.add_patch(box_fn)
    ...
    # Bootstrap icon
    circ = Circle((2.65, y_boot), 0.22, facecolor=C_GOLD_L, edgecolor=C_GOLD, linewidth=1)
    ax.add_patch(circ)
    ...
    draw_arrow(ax, 2.88, y_boot, 3.55, y_boot, color=C_SUBTEXT, lw=1)
    ...
    for x, y, label, text_c, fill_c, ptext in categories:
        box = FancyBboxPatch((x - 0.48, y - 0.10), 0.96, 0.20,
                             ...
                             facecolor=fill_c, edgecolor=text_c, linewidth=1)
        ax.add_patch(box)
    ...
    box_t = FancyBboxPatch((0.55, y_app - 0.45), 1.7, 0.95, boxstyle='round,pad=0.05,rounding_size=0.1',
                           facecolor=C_ORANGE_L, edgecolor=C_ORANGE, linewidth=1)
    ax.add_patch(box_t)
    ax.text(1.40, y_app + 0.38, 'TCGA cancer', fontsize=9, color=C_ORANGE, ha='center', va='top', fontweight='bold')
    ax.text(1.40, y_app + 0.28, 'Tumor vs. normal', fontsize=6.5, color=C_SUBTEXT, ha='center', va='top')
    ...
    ax.text(1.15, y_app - 0.12, 'Tumor', fontsize=6, color=C_TEXT, ha='center', va='top')
    ax.text(1.65, y_app - 0.12, 'Normal', fontsize=6, color=C_TEXT, ha='center', va='top')

    # Brain atlas panel
    box_b = FancyBboxPatch((2.40, y_app - 0.45), 1.7, 0.95, boxstyle='round,pad=0.05,rounding_size=0.1',
                           facecolor=C_BLUE_L, edgecolor=C_BLUE, linewidth=1)
    ax.add_patch(box_b)
    ax.text(3.25, y_app + 0.38, 'Human brain atlas', fontsize=9, color=C_BLUE, ha='center', va='top', fontweight='bold')
    ax.text(3.25, y_app + 0.28, 'Cell-type diversity', fontsize=6.5, color=C_SUBTEXT, ha='center', va='top')
    ...
    box_m = FancyBboxPatch((4.25, y_app - 0.45), 1.7, 0.95, boxstyle='round,pad=0.05,rounding_size=0.1',
                           facecolor=C_GREEN_L, edgecolor=C_GREEN, linewidth=1)
    ax.add_patch(box_m)
    ax.text(5.10, y_app + 0.38, 'Tabula atlases', fontsize=9, color=C_GREEN, ha='center', va='top', fontweight='bold')
    ax.text(5.10, y_app + 0.28, 'Validation across tissues', fontsize=6.5, color=C_SUBTEXT, ha='center', va='top')
